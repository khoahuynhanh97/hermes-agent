...
## Audit Notes (2026-08-05)

### Architecture Facts (vs branch codex/hermes-personal-assistant-core, base 4cba6d81e)

Fact 1: 	elegram_bot.py is the sole Telegram entry � no 	elegram_gateway.py wrapper, no multi-platform support. Hermes Agent (NousResearch) has multi-platform gateway; repo doesn't wrap it.

Fact 2: core/assistant_runtime.py:classify (line 130-143) uses deterministic keyword scoring. 	elegram_bot.py:is_product_research_script_request (line 17-21) uses keyword AND. No LLM reasoning loop.

Fact 3: core/llm_gateway.py (1354 lines) and hermes/llm.py (101 lines) are two parallel LLM gateway layers. core/llm_gateway.py is the lower-level OpenAI-compatible client; hermes/llm.py is the typed wrapper.

Fact 4: Three polling-based worker loops: scripts/run_job_worker.py, scripts/affiliate_research_worker.py, core/job_watcher.py. Hermes Agent has built-in cron scheduler; repo doesn't use it.

Fact 5: 	elegram_bot.py:handle_callback (line 2332) is the custom HITL gate. Inline approve/reject buttons. Hermes Agent has pprovals.mode built-in; repo doesn't use it.

Fact 6: Knowledge base is native SQLite FTS5 (hermes/knowledge.py, 44KB). 39+40 static skills. No graphify integration. graphify skill installed at .agents/skills/graphify/SKILL.md is upstream copy, not repo's own.

Fact 7: Root-level scripts ffiliate_worker.py and 	elegram_notifier.py overlap with scripts/affiliate_research_worker.py and 	elegram_bot.py. Likely legacy before atomic refactor.

### Module Relationships

- hermes/application/product_research_script_workflow.py orchestrates: source ? import ? scoring ? script generation ? local+sheets export
- hermes/application/product_research_intent.py parses Vietnamese natural commands
- hermes/application/product_source_selector.py gates crawler via marketplace_crawler_enabled flag
- hermes/adapters/local/sheet_projection.py writes CSV/XLSX locally
- hermes/adapters/google/sheets_projection.py writes to Google Sheets (optional)
- core/assistant_runtime.py routes messages to product_research_script module

### Refactor Candidates (decision deferred to Plan)

- Consolidate core/llm_gateway.py + hermes/llm.py into single layer
- Merge scripts/run_job_worker.py + scripts/affiliate_research_worker.py into one configurable worker
- Remove root-level ffiliate_worker.py and 	elegram_notifier.py (legacy)
- Move 	elegram_bot.md to docs/ or delete (it's a generated snapshot, not maintained)
- Confirm data/skills/ doesn't exist; the 40 skills are platform-level (.agents/skills/, ~/.claude/skills/)

### Refused

- data/skills/graphify doesn't exist � graphify skill is at .agents/skills/graphify/ (upstream copy)
- graphify add -p doesn't support free-text fact persistence; documented facts in GRAPH_REPORT.md instead
- D:\work\Hermes-super-assistant doesn't exist; prompt was re-interpreted for current D:\work\hermes-agent repo
