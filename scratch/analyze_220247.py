...
def analyze_220247_assembly():
    ...
    frames_dir = r"C:\Users\TeamSol\Downloads\TIKTOK\xe_may\extracted_frames\VID_20260703_220247_detailed"
    selected_frames = [
        "frame_0000_t0.00.jpg",
        "frame_0150_t5.01.jpg",
        "frame_0300_t10.03.jpg",
        "frame_0450_t15.04.jpg",
        "frame_0600_t20.05.jpg",
        "frame_0750_t25.06.jpg",
        "frame_0900_t30.08.jpg",
        "frame_1050_t35.09.jpg",
        "frame_1200_t40.10.jpg",
        "frame_1350_t45.11.jpg",
        "frame_1500_t50.13.jpg"
    ]
    
    parts_to_send = []
    
    print("Preparing key detailed frames for VID_20260703_220247...")
        ...
        if os.path.exists(frame_path):
            ...
            if base64_data:
                parts_to_send.append({
                    "text": f"Frame timestamp: {frame_name.split('_t')[1].replace('.jpg','')}"
                })
                parts_to_send.append({
                    "inlineData": {
                        "mimeType": "image/jpeg",
...
Dưới đây là một số hình ảnh chi tiết được trích xuất từ video "VID_20260703_220247.mp4" (video lắp ráp chính dài 52 giây).

Nhiệm vụ của bạn:
1. Dựa trên các hình ảnh này, hãy phân tích chi tiết quy trình lắp ráp thực tế của sản phẩm được ghi lại trong video này.
   - Nhận diện chính xác xem trong video này bàn tay đang làm các hành động gì ở các mốc thời gian (ví dụ: đang kết nối phần nào vào phần nào, đang vặn con ốc nào, gắn mũ bảo hiểm mini khi nào).
   - Sản phẩm chỉ có 4 bộ phận chính: Thân đỡ điện thoại chính màu đen, Mũ bảo hiểm mini màu đen kính trắng, Đế nhựa gắn màu trắng hình vuông, Cần nối tay đỡ màu đen dạng cong và 2 con ốc (1 đen, 1 trắng).
2. Tạo lại kịch bản phân cảnh (Storyboard AI) gồm 6 cảnh, thời lượng 24 giây (mỗi cảnh 4 giây), phong cách TikTok hiện đại để giới thiệu quy trình lắp ghép chính xác theo video này.
...
Hãy trả về kết quả dưới định dạng JSON với cấu trúc chính xác sau đây (không bọc trong thẻ markdown, trả về JSON thuần):
{
  "product_analysis": {
    ...
    "assembly_steps": [
      "Bước 1 chi tiết lắp ráp trong video...",
      "Bước 2...",
      "Bước 3...",
      "Bước 4..."
    ]
  },
  "title": "Storyboard quảng cáo lắp ghép giá đỡ điện thoại có mũ bảo hiểm mini theo video 220247",
...
def save_outputs(storyboard_data, output_dir):
    ...
    md_content = f"# Báo cáo Phân tích & Storyboard Video AI: Lắp Ghép Giá Đỡ Điện Thoại Có Mũ Bảo Hiểm Mini (Chi Tiết Video 220247)\n\n"
    md_content += f"## 1. Phân tích quy trình lắp ráp trong video 220247\n"
    ...
    md_content += "\n### Quy trình lắp ráp chi tiết ghi nhận từ video 220247:\n"
    ...
    with open(prompts_path, 'w', encoding='utf-8') as f:
        f.write("=== TẬP HỢP PROMPT PHỤC VỤ VỊ TRÍ TẠO ẢNH / VIDEO AI (CẬP NHẬT THEO VIDEO 220247) ===\n\n")
...
if __name__ == "__main__":
    storyboard = analyze_220247_assembly()
    if storyboard:
