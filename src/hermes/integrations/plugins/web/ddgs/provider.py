...
def _plugins_path_entry() -> str:
    """Return the ``sys.path`` entry that makes ``from hermes.integrations import plugins`` work.
    ...
    """
    try:
        from hermes.integrations import plugins as plugins_pkg
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    *its* GIL — it only polls a communicator thread and, on timeout/interrupt,
    ...
    # which breaks ``from hermes.integrations import plugins...``. Prepend the path entry that makes the
