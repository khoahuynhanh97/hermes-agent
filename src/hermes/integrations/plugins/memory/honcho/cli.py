...
from hermes.integrations.plugins.memory.honcho.client import _host_block, profile_host_key, resolve_active_host, resolve_config_path, HOST
...
def _ensure_peer_exists(host_key: str | None = None) -> bool:
    ...
    """
    try:
        from hermes.integrations.plugins.memory.honcho.client import HonchoClientConfig, get_honcho_client
...
def cmd_setup(args) -> None:
    ...
    if not is_local:
        ...
        from hermes.integrations.plugins.memory.honcho.oauth import OAuthCredential
        ...
        if use_device:
            from hermes.integrations.plugins.memory.honcho.oauth_flow import (
                AccessDenied,
            ...
            from hermes.integrations.plugins.memory.honcho.oauth_flow import authorize_via_loopback
    ...
    try:
        from hermes.integrations.plugins.memory.honcho.client import HonchoClientConfig, get_honcho_client, reset_honcho_client
        reset_honcho_client()
...
def _device_login_available() -> bool:
    ...
    try:
        from hermes.integrations.plugins.memory.honcho.oauth_flow import resolve_endpoints, supports_device_login
...
def _show_peer_cards(hcfg, client) -> None:
    ...
    """
    try:
        from hermes.integrations.plugins.memory.honcho.session import HonchoSessionManager
...
def cmd_migrate(args) -> None:
    ...
    if user_files:
        ...
        if has_key:
            ...
            if answer.lower() in {"y", "yes"}:
                try:
                    from hermes.integrations.plugins.memory.honcho.client import (
                        HonchoClientConfig,
                        get_honcho_client,
                        reset_honcho_client,
                    )
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from hermes.channels.cli.memory_setup import cmd_setup_provider
