...
from hermes.channels.cli.profiles import _get_default_hermes_home
from hermes.integrations.plugins.plugin_utils import SingletonSlot
...
class HonchoClientConfig:
    ...
    def resolve_session_name(
        ...
        Resolution order:
          1. Gateway session key (stable per-chat identifier from hermes.channels.gateway platforms)
...
def _apply_fresh_oauth_token(config: HonchoClientConfig) -> None:
    ...
    """
    try:
        from hermes.integrations.plugins.memory.honcho import oauth
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    def _build() -> "Honcho":
        ...
        try:
            from hermes.tools.lazy_deps import FeatureUnavailable, ensure as _lazy_ensure
