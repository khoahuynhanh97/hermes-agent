...
from hermes.channels.cli import kanban_db
from hermes.channels.cli import kanban_diagnostics as kd
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    also means this can never drift from hermes.application.core auth again.
    ...
    """
    try:
        from hermes.channels.cli import web_server as _ws
    except Exception:
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from hermes.channels.cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from hermes.channels.cli.kanban import _check_dispatcher_presence
...
from hermes.channels.cli.kanban_db import (  # noqa: E402
...
def specify_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from hermes.channels.cli import kanban_specify  # noqa: WPS433 (intentional)
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from hermes.runtime.constants import get_hermes_home  # type: ignore
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from hermes.channels.cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from hermes.channels.cli import kanban_decompose  # noqa: WPS433 (intentional)
