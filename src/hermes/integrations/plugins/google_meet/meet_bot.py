...
def _start_realtime_speaker(
    *,
    ...
    """
    try:
        from hermes.integrations.plugins.google_meet.realtime.openai_client import (
            RealtimeSession,
            RealtimeSpeaker,
        )
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            ...
            try:
                from hermes.integrations.plugins.google_meet.audio_bridge import AudioBridge
                bridge = AudioBridge()
