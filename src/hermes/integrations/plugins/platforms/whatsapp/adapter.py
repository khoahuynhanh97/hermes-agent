...
def _bridge_pid_is_ours(pid: int, session_path: Path, expected_start) -> bool:
    ...
    if expected_start is not None:
        from hermes.channels.gateway.status import get_process_start_time
    ...
    from hermes.channels.gateway.status import _read_process_cmdline
...
from hermes.channels.gateway.platforms.whatsapp_common import WhatsAppBehaviorMixin
from hermes.channels.gateway.whatsapp_identity import to_whatsapp_jid
...
#
# Added when the WhatsApp adapter moved from hermes.channels.gateway/platforms/whatsapp.py into
...
def _apply_yaml_config(yaml_cfg: dict, whatsapp_cfg: dict) -> dict | None:
    ...
    whatsapp_cfg block from hermes.channels.gateway/config.py::load_gateway_config(). Env vars
