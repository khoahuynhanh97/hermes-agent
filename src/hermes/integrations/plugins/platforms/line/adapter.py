...
class LineAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            from hermes.channels.gateway.status import acquire_scoped_lock
...
async def _standalone_send(
    ...
    *,
    ...
    if media_files:
        ...
        messages.append(_text_message(f"[{len(media_files)} attachment(s) generated; not deliverable from hermes.scheduling]"))
...
def interactive_setup() -> None:
    ...
    try:
        from hermes.channels.cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
