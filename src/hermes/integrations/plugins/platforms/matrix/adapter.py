...
from hermes.channels.gateway.platforms.helpers import ThreadParticipationTracker
...
from hermes.runtime.constants import get_hermes_dir as _get_hermes_dir
...
def check_matrix_requirements() -> bool:
    ...
    try:
        from hermes.tools.lazy_deps import feature_missing, ensure_and_bind
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def _verify_device_keys_on_server(self, client: Any, olm: Any) -> bool:
        ...
        if not our_keys:
            logger.warning("Matrix: device keys missing from hermes.channels.api — re-uploading")
        ...
        if server_ed25519 != local_ed25519:
            ...
            try:
                ...
                logger.info(
                    "Matrix: deleted stale device %s from hermes.channels.api", client.device_id
                )
            except Exception:
                pass
            try:
...
#
# Added when the Matrix adapter moved from hermes.channels.gateway/platforms/matrix.py into
...
def interactive_setup() -> None:
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        try:
            from hermes.tools.lazy_deps import ensure as _lazy_ensure, feature_missing
...
def _apply_yaml_config(yaml_cfg: dict, matrix_cfg: dict) -> dict | None:
    ...
    matrix_cfg block from hermes.channels.gateway/config.py::load_gateway_config(). Env vars
