...
from hermes.integrations.providers.gemini_common import (
    build_contents,
    extract_image,
    generation_config,
    output_path_for,
)
from hermes.integrations.providers.vertex_auth import get_access_token, vertex_model_endpoint, vertex_required_project
...
class GoogleVertexImageProvider(ImageGenerationPort):
    ...
    def __init__(
        ...
        configured_model = (model or os.environ.get("IMAGE_MODEL", "")).strip() or "gemini-2.5-flash-image"
        self.model = DEPRECATED_IMAGE_MODELS.get(configured_model, configured_model)
