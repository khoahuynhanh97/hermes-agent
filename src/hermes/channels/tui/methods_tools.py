"""Tools & system / slash.exec / insights / rollback / browser-plugins-cron-skills JSON-RPC handlers (moved verbatim from hermes.channels.api.py).
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from hermes.agent.battery import battery_category, read_battery
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from hermes.tools.mcp_tool import shutdown_mcp_servers, discover_mcp_tools
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hermes.channels.cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hermes.agent.skill_bundles import (
    ...
    if name == "moa":
        ...
        try:
            from hermes.channels.cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hermes.agent.skill_bundles import resolve_bundle_command_key
        from hermes.channels.cli.commands import resolve_command
    ...
    try:
        from hermes.agent.skill_commands import get_skill_commands
...
def _(rid, params: dict) -> dict:
    try:
        from hermes.agent.model_tools import get_toolset_for_tool, get_tool_definitions
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hermes.tools.cronjob_tools import cronjob
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from hermes.agent.learning_graph_render import render_frames
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "search":
            from hermes.tools.skills_hub import (
                GitHubAuth,
        ...
        if action == "install":
            from hermes.channels.cli.skills_hub import do_install
        ...
        if action == "browse":
            from hermes.channels.cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from hermes.channels.cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    if not cmd:
        return _err(rid, 4004, "empty command")
    try:
        from hermes.tools.approval import detect_dangerous_command, detect_hardline_command
