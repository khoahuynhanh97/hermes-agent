...
from hermes.tools.environments.local import hermes_subprocess_env
from hermes.agent.replay_cleanup import sanitize_replay_history
from hermes.agent.skill_commands import describe_skill_invocation
from hermes.agent.conversation_loop import INTERRUPT_WAITING_FOR_MODEL_PREFIX
from hermes.channels.tui import git_probe
from hermes.channels.tui.turn_marker import (
    clear_turn_marker,
    read_turn_marker,
    record_turn_start,
)
from hermes.channels.tui.transport import (
    StdioTransport,
    Transport,
    bind_transport,
    current_transport,
...
try:
    from hermes.channels.cli.banner import prefetch_update_check

    prefetch_update_check()
except Exception:
    pass

from hermes.channels.tui.render import make_stream_renderer, render_diff, render_message
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from hermes.channels.cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from hermes.channels.cli.lifecycle import invoke_hook
    ...
    # Use session_id (from hermes.agent.session_id) not session_key — after compression,
    ...
    try:
        from hermes.tools.async_delegation import interrupt_for_session
...
def _teardown_session(session: dict | None, *, end_reason: str = "tui_close") -> None:
    ...
    try:
        from hermes.tools.approval import unregister_gateway_notify
...
def _session_has_active_delegations(sid: str, session: dict | None = None) -> bool:
    ...
    if not own_sid and not owned_session_key:
        return False
    try:
        from hermes.tools.async_delegation import has_live_for_session
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from hermes.channels.cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from hermes.channels.cli.active_sessions import coerce_max_concurrent_sessions
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from hermes.channels.cli.config import _expand_env_vars, read_user_config_raw
...
def _get_compute_host_supervisor(cfg: dict | None = None):
    ...
    with _compute_host_supervisor_lock:
        if _compute_host_supervisor is None:
            from hermes.channels.tui.host_supervisor import HostSupervisor
...
def _emit_approval_request(sid: str, data: dict | None) -> None:
    ...
    if "command" in payload:
        from hermes.channels.gateway.run import _redact_approval_command
...
def _status_update(sid: str, kind: str, text: str | None = None):
    ...
    if out_kind == "lifecycle":
        from hermes.agent.conversation_compression import COMPACTION_STATUS_MARKER
    ...
    def _build() -> None:
        ...
        try:
            ...
            if profile_home:
                ...
                try:
                    from hermes.agent.secret_scope import build_profile_secret_scope, set_secret_scope
            ...
            try:
                from hermes.channels.tui.entry import ensure_mcp_discovery_started
            ...
            try:
                from hermes.agent.credits_tracker import seed_credits_at_session_start
            ...
            if secret_token is not None:
                try:
                    from hermes.agent.secret_scope import reset_secret_scope
...
def _reconcile_session_cwd_from_terminal(session: dict | None) -> bool:
    ...
    try:
        from hermes.tools.terminal_tool import get_session_cwd
...
def _register_session_cwd(session: dict | None) -> None:
    if not session:
        return
    try:
        from hermes.tools.terminal_tool import register_task_env_overrides
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``hermes`` / ``hermes --tui`` / CLI) is started from a
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from hermes.channels.cli.runtime_provider import canonical_custom_identity
    ...
    try:
        ...
        from hermes.state.service import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from hermes.runtime.constants import translate_cwd_for_wsl_backend
    ...
    _persist_session_git_meta(session, resolved)
    try:
        from hermes.tools.terminal_tool import cleanup_vm
...
def _save_cfg(cfg: dict):
    ...
    from hermes.channels.cli.config import atomic_config_write
...
def _set_session_context(
    ...
    *,
    ...
    try:
        from hermes.channels.gateway.session_context import set_session_vars
...
def _clear_session_context(tokens: list) -> None:
    if not tokens:
        return
    try:
        from hermes.channels.gateway.session_context import clear_session_vars
...
def _clarify_timeout_seconds() -> float | None:
    ...
    try:
        from hermes.tools.clarify_gateway import get_clarify_timeout
...
def resolve_skin() -> dict:
    try:
        from hermes.channels.cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    ...
    try:
        from hermes.channels.cli.models import get_preferred_silent_default_model
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from hermes.channels.cli.models import detect_static_provider_for_model
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from hermes.channels.cli.runtime_provider import (
...
def _load_approval_mode() -> str:
    ...
    not fully mirror).
    """
    from hermes.tools.approval import _get_approval_mode
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    try:
        from hermes.tools.toolsets import validate_toolset
    except Exception:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            from hermes.channels.cli.config import read_raw_config
            from hermes.channels.cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(
    ...
    *,
    ...
    from hermes.channels.cli.runtime_provider import resolve_runtime_provider
    ...
    cfg = None
    try:
        from hermes.channels.cli.config import get_compatible_custom_providers, load_config
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from hermes.channels.cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        from hermes.tools.approval import _YOLO_MODE_FROZEN, is_session_yolo_enabled
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            from hermes.agent.model_tools import get_toolset_for_tool
            ...
            pass
        try:
            from hermes.channels.cli.banner import get_available_skills

            info["skills"] = get_available_skills()
        except Exception:
            pass
    try:
        from hermes.tools.mcp_tool import get_mcp_status
    ...
    try:
        ...
        pass
    try:
        from hermes.channels.cli.banner import get_update_result
        from hermes.channels.cli.config import recommended_update_command
...
def _tool_ctx(name: str, args: dict) -> str:
    ...
    """
    try:
        from hermes.agent.display import build_tool_preview
...
def _tool_result_text(result: object) -> str:
    try:
        from hermes.agent.tool_dispatch_helpers import _multimodal_text_summary
...
def _wire_callbacks(sid: str):
    from hermes.tools.terminal_tool import set_sudo_password_callback
    from hermes.tools.skills_tool import set_secret_capture_callback
    from hermes.tools.project_tools import set_project_workspace_callback
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from hermes.channels.cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from hermes.channels.cli.config import load_config as _load_full_cfg
...
def _schedule_mcp_late_refresh(sid: str, agent) -> None:
    ...
    """
    try:
        from hermes.channels.tui.entry import mcp_discovery_in_flight, join_mcp_discovery
    except Exception:
        return
...
def _resolve_runtime_with_fallback(
    ...
    """
    from hermes.channels.cli.auth import AuthError
    from hermes.channels.cli.runtime_provider import resolve_runtime_provider
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from hermes.channels.cli.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    from hermes.channels.tui.synthetic_turn import maybe_build_synthetic_agent
    ...
    try:
        ...
        pass
    try:
        from hermes.channels.tui.entry import wait_for_mcp_discovery
    ...
    if startup_skills:
        from hermes.agent.skill_commands import build_preloaded_skills_prompt
...
def _init_session(
    ...
    try:
        from hermes.tools.approval import register_gateway_notify, load_permanent_allowlist
...
def _active_image_routing_identity(agent: Any) -> tuple[str, str]:
    ...
    from hermes.agent.auxiliary_client import _read_main_model, _read_main_provider
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    """
    from hermes.agent.context_references import format_reference_value
...
def _pet_frame_counts(spritesheet) -> dict:
    ...
    """
    try:
        from hermes.agent.pet import render
...
def _pet_row_frame_counts(spritesheet) -> dict:
    ...
    try:
        ...
        from hermes.agent.pet import constants, render
...
def _pet_config_scale() -> float:
    ...
    from hermes.agent.pet import constants
...
def _serialize_billing_error(exc) -> dict:
    ...
    from hermes.channels.cli.nous_billing import (
...
def _serialize_billing_state(state) -> dict:
    ...
    from hermes.agent.billing_view import format_money
...
def _usage_payload(state) -> dict:
    ...
    try:
        from hermes.agent.billing_usage import build_usage_model
...
def _serialize_usage_bar(bar) -> Optional[dict]:
    ...
    if bar is None:
        return None
    from hermes.agent.billing_usage import _fmt_usd
...
def _serialize_usage_model(model) -> dict:
    ...
    """
    from hermes.agent.billing_usage import _fmt_usd, format_renews
...
def _serialize_subscription_state(state) -> dict:
    ...
    from hermes.agent.billing_usage import format_renews
    from hermes.agent.billing_view import format_money
...
def _notification_poller_loop(
    ...
    """
    from hermes.tools.process_registry import process_registry, format_process_notification
...
def _wire_desktop_ui() -> None:
    ...
    if _desktop_ui_wired:
        return
    try:
        from hermes.tools import desktop_ui
    except Exception:
        return
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from hermes.channels.cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from hermes.channels.cli.voice import is_continuous_active
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from hermes.channels.cli.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from hermes.channels.cli.models import resolve_fast_mode_overrides
    ...
    if key == "yolo":
        ...
        try:
            ...
            if scope == "global":
                from hermes.tools.approval import _normalize_approval_mode
...
def _repo_discovery_policy(raw: dict | None = None) -> dict:
    ...
    from hermes.channels.cli.config import DEFAULT_CONFIG
...
def _build_project_tree(
    ...
    from hermes.channels.tui import project_tree
...
def _skill_usage_lookup():
    ...
    """
    try:
        from hermes.tools.skill_usage import (
...
def _model_picker_context(agent):
    ...
    from hermes.channels.cli.inventory import load_picker_context
...
def _tts_stream_begin() -> Optional[queue.Queue]:
    ...
    if not _voice_tts_enabled():
        return None
    try:
        from hermes.tools.tts_tool import check_tts_requirements, stream_tts_to_speaker
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    try:
                        from hermes.channels.cli.voice import stop_continuous
...
def _speak_text_with_barge(text: str) -> None:
    ...
    """
    from hermes.channels.cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    def _on_detect() -> None:
        from hermes.tools.wake_word import get_last_match, owns_listener, pause_listening
...
def _(rid, params: dict) -> dict:
    ...
    if bool(params.get("persist")):
        try:
            from hermes.tools.wake_word import load_wake_word_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hermes.tools.wake_word import feed_audio
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from hermes.channels.cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from hermes.channels.cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import hermes.channels.cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from hermes.channels.cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from hermes.channels.cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    from hermes.tools.browser_tool import cleanup_all_browsers
