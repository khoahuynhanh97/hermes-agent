...
from hermes.runtime import bootstrap as hermes_bootstrap
...
from hermes.channels.tui._stdin_recovery import handle_spurious_eof

from hermes.channels.tui import server
from hermes.channels.tui.server import _CRASH_LOG, dispatch, resolve_skin, write_json
from hermes.channels.tui.transport import TeeTransport
...
def _install_sidecar_publisher() -> None:
    ...
    from hermes.channels.tui.event_publisher import WsPublisherTransport
...
def _log_signal(signum: int, frame) -> None:
    ...
    try:
        from hermes.channels.tui.server import _shutdown_sessions
...
# ``from hermes.channels.tui.entry import ensure_mcp_discovery_started`` as the first
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    if thread is not None and thread.is_alive():
        try:
            from hermes.channels.cli.mcp_startup import _resolve_discovery_timeout
    ...
    try:
        from hermes.channels.cli.mcp_startup import (
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    try:
        from hermes.channels.cli.mcp_startup import join_mcp_discovery as _startup_join
