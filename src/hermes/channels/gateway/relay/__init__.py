...
def relay_url() -> Optional[str]:
    ...
    if url:
        return url.rstrip("/")
    try:
        from hermes.channels.gateway.run import _load_gateway_config  # late import to avoid cycle
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from hermes.channels.cli.skin_engine import get_active_skin  # late import: boot-safe
...
def _resolve_relay_identity_token() -> str:
    ...
    if not token_url:
        ...
        from hermes.channels.cli.auth import resolve_nous_access_token
...
def send_relay_policy() -> bool:
    ...
    try:
        from hermes.channels.gateway.relay.auth import make_upgrade_token
...
def register_relay_adapter(force: bool = False, url: Optional[str] = None) -> bool:
    ...
    from hermes.channels.gateway.platform_registry import PlatformEntry, platform_registry
    from hermes.channels.gateway.relay.adapter import RelayAdapter
    from hermes.channels.gateway.relay.descriptor import CONTRACT_VERSION, CapabilityDescriptor
    ...
    def _factory(config):
        ...
        transport = None
        if resolved_url:
            from hermes.channels.gateway.relay.ws_transport import WebSocketRelayTransport
