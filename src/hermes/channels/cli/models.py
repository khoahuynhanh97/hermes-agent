...
from hermes.channels.cli.urllib_security import open_credentialed_url
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from hermes.channels.cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def _xai_curated_models() -> list[str]:
    ...
    """
    try:
        from hermes.agent.models_dev import _load_disk_cache
...
try:
    from hermes.integrations.providers import list_providers as _list_providers_for_canonical
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from hermes.channels.cli.model_catalog import get_default_model_from_cache
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from hermes.channels.cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from hermes.channels.cli.model_catalog import get_curated_nous_models
...
def compute_sale_discount(
    ...
    """Derive sale chrome from hermes.channels.gateway ``pricing.original`` when cheaper.
...
def _fireworks_pricing_from_models_dev(
    *,
    ...
    result: dict[str, dict[str, str]] = {}
    try:
        from hermes.agent.models_dev import _get_provider_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from hermes.channels.cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from hermes.channels.cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from hermes.channels.cli.auth import resolve_api_key_provider_credentials
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from hermes.channels.cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from hermes.channels.cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``hermes model`` shows.
            from hermes.channels.cli.providers import is_official_openai_host
    ...
    if normalized == "bedrock":
        try:
            from hermes.agent.bedrock_adapter import bedrock_model_ids_or_none
...
def _fetch_anthropic_models(
    ...
    *,
    ...
    """
    try:
        from hermes.agent.anthropic_adapter import resolve_anthropic_token, _is_oauth_token
    except ImportError:
        return None
    ...
    if is_oauth:
        ...
        from hermes.agent.anthropic_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "bedrock":
        try:
            from hermes.agent.bedrock_adapter import discover_bedrock_models, resolve_bedrock_region
