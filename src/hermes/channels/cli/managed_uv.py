...
from hermes.channels.cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime
...
def _reload_hermes_constants():
    ...
    """
    from hermes.runtime import constants as hermes_constants
...
def _smoke_candidate_venv(venv_dir: Path) -> tuple[bool, str, SQLiteRuntimeInfo | None]:
    ...
    check = (
        ...
        "from hermes.state import service as hermes_state\n"
    )
    env = dict(os.environ)
    for key in (
        "CONDA_DEFAULT_ENV",
