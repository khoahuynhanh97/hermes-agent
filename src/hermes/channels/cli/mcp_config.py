...
from hermes.tools.mcp_tool import _ENV_VAR_PATTERN, _env_ref_name
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from hermes.channels.cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    """
    from hermes.tools.mcp_tool import _interpolate_env_vars

    from hermes.agent.secret_scope import current_secret_scope
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    """
    try:
        from hermes.tools.mcp_oauth import HermesTokenStorage
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    try:
        from hermes.tools.mcp_oauth import force_interactive_oauth
...
def cmd_mcp_configure(args):
    ...
    try:
        from hermes.tools.mcp_tool import matches_name_filter
...
def mcp_command(args):
    ...
    if action == "serve":
        from hermes.mcp.serve import run_mcp_server
    ...
    if action == "picker":
        from hermes.channels.cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from hermes.channels.cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from hermes.channels.cli.mcp_picker import install_by_name
        import sys as _sys
