...
def _redact_sensitive_cmdline(cmdline: str) -> str:
    ...
    try:
        from hermes.agent.redact import redact_sensitive_text  # noqa: PLC0415
...
def _is_pausable_gateway(cmdline: str) -> bool:
    ...
    *value* shadowed the subcommand token. An import failure counts as
    ...
    """
    try:
        from hermes.channels.gateway.status import looks_like_gateway_command_line  # noqa: PLC0415
    except Exception:
        return False
...
def main() -> None:
    ...
    try:
        from hermes.channels.cli.main import _detect_venv_python_processes  # noqa: PLC0415
