...
from hermes.channels.cli.cli_output import prompt_yes_no
from hermes.channels.cli.curses_ui import curses_single_select
from hermes.channels.cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from hermes.channels.cli.mcp_config import cmd_mcp_configure
...
def _remove_custom(name: str) -> None:
    ...
    if name not in servers:
        ...
        return
    if not prompt_yes_no(f"Remove '{name}' from hermes.mcp?", default=False):
        return
    del servers[name]
    if not servers:
...
def install_by_name(identifier: str) -> int:
    ...
    exit codes).
    """
    from hermes.channels.cli.mcp_catalog import get_entry
