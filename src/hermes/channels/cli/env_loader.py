...
def _known_hermes_env_keys() -> set[str]:
    ...
    """
    from hermes.channels.cli.config import _EXTRA_ENV_KEYS
    from hermes.channels.cli.config_defaults import OPTIONAL_ENV_VARS
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        from hermes.agent.secret_scope import _is_global_env, load_env_file
        from hermes.agent.secret_sources.registry import apply_all
...
def format_secret_source_suffix(env_var: str) -> str:
    ...
    try:
        from hermes.agent.secret_sources.registry import get_source
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from hermes.channels.cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    if project_env:
        ...
        from hermes.runtime_layout import canonical_repo_root
        project_env_path = canonical_repo_root() / ".env"
