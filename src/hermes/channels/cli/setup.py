...
from hermes.tools.tool_backend_helpers import managed_nous_tools_enabled
from hermes.runtime.constants import get_optional_skills_dir
...
from hermes.channels.cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from hermes.channels.cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        from hermes.channels.cli.auth import resolve_provider
    ...
    try:
        from hermes.agent.auxiliary_client import get_available_vision_backends
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            from hermes.agent.video_gen_registry import list_providers as _list_video_providers
            from hermes.channels.cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from hermes.channels.cli.main import select_provider_and_model
    try:
...
def _install_neutts_deps() -> bool:
    ...
    from hermes.channels.cli.tools_config import _pip_install
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        from hermes.tools.managed_tool_gateway import is_managed_tool_gateway_ready
        from hermes.tools.tool_backend_helpers import normalize_modal_mode
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # HERMES_MAX_ITERATIONS from hermes.agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from hermes.channels.cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_qqbot():
    ...
    from hermes.channels.cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from hermes.channels.cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    """
    from hermes.channels.cli.tools_config import tools_command
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from hermes.channels.cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from hermes.channels.cli.config
    ...
    if section_key == "model":
        ...
        from hermes.channels.cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from hermes.channels.cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from hermes.channels.cli.config import is_managed, managed_error
    if is_managed():
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        from hermes.tools.toolsets import TOOLSETS
        from hermes.channels.cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        try:
            from hermes.tools.skills_sync import set_bundled_skills_opt_out
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        from hermes.tools.skills_sync import set_bundled_skills_opt_out, sync_skills
        if seed_skills:
