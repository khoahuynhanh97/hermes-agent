...
from hermes.channels.cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from hermes.channels.cli import __version__, __release_date__
...
from hermes.integrations.plugins.memory.config_schema import (
    ProviderConfigSchema,
    ProviderField,
    STORAGE_HONCHO_HOST_BLOCK,
    get_provider_config_schema,
)
from hermes.channels.gateway.status import (
    derive_gateway_busy,
...
from hermes.runtime.utils import env_var_enabled
...
try:
    ...
    try:
        from hermes.tools.lazy_deps import ensure as _lazy_ensure
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    first wins the tick.
    """
    from hermes.scheduling.scheduler_provider import resolve_cron_scheduler
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from hermes.channels.cli.gateway import _get_restart_drain_timeout
        return _get_restart_drain_timeout()
    except ImportError:
        from hermes.channels.gateway.restart import DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
...
from hermes.channels.cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ---------------------------------------------------------------------------
from hermes.channels.cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from hermes.channels.cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from hermes.channels.cli.dashboard_auth.token_auth import token_auth_middleware
...
def _memory_provider_options() -> List[str]:
    ...
    options = [""]
    try:
        from hermes.integrations.plugins.memory import list_memory_provider_names
...
def _custom_provider_options(
    ...
    *kind* is ``"tts"`` or ``"stt"``. The result keeps the built-in display
    ...
    if kind == "tts":
        from hermes.tools.tts_tool import BUILTIN_TTS_PROVIDERS as _runtime_builtins
    else:
        from hermes.tools.transcription_tools import BUILTIN_STT_PROVIDERS as _runtime_builtins
    ...
    try:
        if kind == "tts":
            from hermes.agent.tts_registry import list_providers as _list_voice_providers
        else:
            from hermes.agent.transcription_registry import list_providers as _list_voice_providers
...
from hermes.channels.cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from hermes.channels.cli.config import get_compatible_custom_providers
    from hermes.channels.cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from hermes.channels.cli.model_normalize import normalize_model_for_provider
    from hermes.channels.cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from hermes.channels.cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    """
    from hermes.state.service import DEFAULT_DB_PATH, SessionDB
...
def _default_hermes_root_is_opt_data() -> bool:
    ...
    if not raw:
        return False
    try:
        from hermes.runtime.constants import get_default_hermes_root
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    if _default_hermes_root_is_opt_data():
        return True
    try:
        from hermes.runtime.constants import is_container
...
from hermes.channels.cli import web_git as _web_git  # noqa: E402
...
from hermes.channels.cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from hermes.channels.cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _profile_platform_ports(profile_home: Path, runtime: Optional[dict]) -> Dict[str, int]:
    ...
    try:
        ...
        from hermes.channels.cli.config import read_user_config_raw
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from hermes.channels.cli.profiles import _check_gateway_running, profiles_to_serve
        from hermes.channels.gateway.status import read_runtime_status
...
def _load_configured_gateway_platforms() -> set[str]:
    ...
    """
    from hermes.channels.gateway.config import load_gateway_config
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        auth_flows: list[str] = []
        try:
            from hermes.channels.cli.dashboard_auth import (
        ...
        try:
            from hermes.channels.cli.auth import get_nous_session_validity
        ...
        try:
            from hermes.channels.gateway.readiness import _probe_state_db
        ...
        try:
            from hermes.state.service import SessionDB as _SDB
...
async def get_curator_status():
    try:
        from hermes.agent import curator
    except Exception as exc:
...
async def get_learning_graph(profile: Optional[str] = None):
    ...
    """
    try:
        from hermes.agent.learning_graph import build_learning_graph
...
async def get_learning_node(id: str, profile: Optional[str] = None):
    ...
    from hermes.agent.learning_mutations import node_detail
...
async def delete_learning_node(body: LearningNodeRef):
    ...
    from hermes.agent.learning_mutations import delete_node
...
async def update_learning_node(body: LearningNodeEdit):
    ...
    from hermes.agent.learning_mutations import edit_node
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from hermes.channels.cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from hermes.channels.cli.nous_subscription import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from hermes.channels.cli.debug import build_debug_share
...
async def gateway_drain(request: Request):
    ...
    """
    from hermes.channels.gateway.drain_control import (
        clear_drain_request,
        drain_requested,
        write_drain_request,
    )
...
async def check_hermes_update(force: bool = False):
    ...
    try:
        from hermes.channels.cli.banner import check_for_updates
...
async def get_elevenlabs_voices(profile: Optional[str] = None):
    ...
    if not api_key:
        ...
        try:
            from hermes.agent.secret_scope import UnscopedSecretError, get_secret
...
def _split_text_for_speak_stream(text: str, cap: int) -> list:
    ...
    """
    from hermes.tools.tts_streaming import SENTENCE_BOUNDARY_RE as _SENTENCE_BOUNDARY_RE
...
async def speak_stream_ws(ws: "WebSocket") -> None:
    ...
    def _resolve():
        from hermes.tools.tts_streaming import resolve_streaming_provider
        from hermes.tools.tts_tool import _get_provider, _load_tts_config, _resolve_max_text_length
    ...
    def _produce():
        from hermes.tools.tts_streaming import SentenceChunker
        from hermes.tools.tts_tool import _strip_markdown_for_tts
...
from hermes.channels.cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from hermes.channels.cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from hermes.channels.cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from hermes.channels.cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _honcho_resolvers():
    ...
    from hermes.integrations.plugins.memory.honcho.client import _host_block, resolve_active_host, resolve_config_path
...
def _write_provider_flat(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    from hermes.runtime.utils import atomic_json_write
...
def _write_provider_honcho(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    ...
    from hermes.integrations.plugins.memory.honcho.oauth import ACCESS_TOKEN_PREFIX, _config_refresh_lock
    from hermes.runtime.utils import atomic_json_write
...
def _load_memory_provider(name: str):
    try:
        from hermes.integrations.plugins.memory import load_memory_provider
...
def _memory_provider_manifest(name: str) -> Dict[str, Any]:
    try:
        from hermes.integrations.plugins.memory import find_provider_dir
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    try:
        from hermes.tools.lazy_deps import install_specs
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            from hermes.agent.memory_provider import MemoryProvider as _BaseMemoryProvider
        except Exception:
...
def _discover_memory_provider_statuses() -> List[Dict[str, Any]]:
    ...
    try:
        from hermes.integrations.plugins.memory import discover_memory_providers
...
async def get_egress_status():
    ...
    from hermes.channels.cli.proxy_cli import format_status_text
...
def get_model_info(profile: Optional[str] = None):
    ...
    try:
        ...
        caps = {}
        try:
            from hermes.agent.models_dev import get_model_capabilities
...
# Mirrors the model.options JSON-RPC from hermes.channels.tui but uses REST so the
...
async def get_model_options(
    ...
    """
    try:
        from hermes.channels.cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            from hermes.channels.cli.models import (
            ...
            from hermes.channels.cli.auth import get_provider_auth_state
    ...
    try:
        ...
        from hermes.channels.cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from hermes.channels.cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from hermes.channels.cli.moa_config import normalize_moa_config, validate_moa_payload
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "nous":
            try:
                from hermes.channels.cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from hermes.channels.cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from hermes.channels.cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from hermes.channels.cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from hermes.channels.cli.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from hermes.channels.cli.credential_lifecycle import remove_provider_env_credential
...
def _messaging_platform_catalog() -> tuple[dict[str, Any], ...]:
    ...
    """
    from hermes.channels.gateway.config import Platform
    ...
    try:
        ...
        discover_plugins()
        from hermes.channels.gateway.platform_registry import platform_registry
...
from hermes.channels.cli.setup_hidden_env import (  # noqa: E402
...
def _gateway_platform_config(platform_id: str):
    from hermes.channels.gateway.config import Platform, load_gateway_config
...
def _whatsapp_session_path() -> Path:
    from hermes.runtime.constants import get_hermes_dir
...
def _ensure_whatsapp_bridge_dependencies(bridge_dir: Path) -> None:
    ...
    from hermes.runtime.constants import find_node_executable, with_hermes_node_path
    from hermes.runtime.utils import env_int
...
def _spawn_whatsapp_pairing_process(session_path: Path, mode: str) -> subprocess.Popen:
    from hermes.channels.gateway.platforms.whatsapp_common import resolve_whatsapp_bridge_dir
    from hermes.runtime.constants import find_node_executable, with_hermes_node_path
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    """
    from hermes.channels.gateway.config import PORT_BINDING_PLATFORM_VALUES, load_gateway_config
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    """
    try:
        from hermes.agent.anthropic_adapter import (
    ...
    try:
        from hermes.channels.cli.auth import PROVIDER_REGISTRY
        env_var_order = PROVIDER_REGISTRY["anthropic"].api_key_env_vars
    except (ImportError, KeyError):
        pass
    try:
        from hermes.channels.cli.config import get_env_value
    except ImportError:
        get_env_value = None  # type: ignore
    try:
        from hermes.channels.cli.env_loader import format_secret_source_suffix
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    """
    try:
        from hermes.agent.anthropic_adapter import read_claude_code_credentials
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from hermes.channels.cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from hermes.agent.anthropic_adapter import _get_hermes_oauth_file
            ...
            try:
                from hermes.channels.cli.auth import clear_provider_auth
        ...
        try:
            from hermes.channels.cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    try:
        from hermes.agent.credential_pool import (
            PooledCredential,
            load_pool,
            AUTH_TYPE_OAUTH,
            SOURCE_MANUAL,
...
async def _start_device_code_flow(
    ...
    """
    if provider_id == "nous":
        from hermes.channels.cli.auth import (
    ...
    if provider_id == "xai-oauth":
        from hermes.channels.cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from hermes.channels.cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from hermes.channels.cli.auth import _save_codex_tokens
...
def _open_session_db_for_profile(profile: Optional[str], *, read_only: bool):
    ...
    from hermes.state.service import SessionDB, _default_db_path, is_malformed_db_error
...
async def get_logs(
    ...
    from hermes.channels.cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from hermes.runtime.logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from hermes.channels.cli import profiles as profiles_mod
    try:
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from hermes.scheduling import jobs as cron_jobs
    from hermes.runtime.constants import (
...
from hermes.channels.cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from hermes.channels.cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _normalize_mcp_server_create(
    ...
    """
    from hermes.channels.cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from hermes.channels.cli.mcp_security import validate_mcp_server_entry
...
from hermes.channels.cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from hermes.channels.cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from hermes.channels.cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        from hermes.agent.secret_scope import (
            build_profile_secret_scope,
            reset_secret_scope,
            set_secret_scope,
        )
        from hermes.runtime.constants import reset_hermes_home_override, set_hermes_home_override
        from hermes.tools.mcp_dashboard_oauth import dashboard_oauth_flow
        from hermes.tools.mcp_oauth import HermesTokenStorage, force_interactive_oauth
        from hermes.tools.mcp_oauth_manager import get_manager
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                try:
                    ...
                    if flow.reconnect_live:
                        from hermes.tools.mcp_tool import reconnect_mcp_server
        ...
        try:
            from hermes.tools.mcp_oauth import humanize_oauth_registration_error
...
def _pairing_store(profile: Optional[str] = None):
    ...
    the ``await`` boundary.
    """
    from hermes.channels.gateway.pairing import PairingStore
...
async def list_webhooks():
    import hermes.channels.cli.webhook as wh
...
async def list_credential_pool():
    from hermes.agent.credential_pool import load_pool
    from hermes.channels.cli.auth import read_credential_pool
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    """
    from hermes.agent.credential_pool import load_pool
    from hermes.agent.credential_sources import find_removal_step
    from hermes.channels.cli.auth import suppress_credential_source
...
async def list_hooks():
    ...
    form can offer them.
    """
    from hermes.channels.cli.config import load_config as _load_config
    from hermes.agent import shell_hooks

    try:
        from hermes.channels.cli.plugins import VALID_HOOKS
...
from hermes.channels.cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from hermes.channels.cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _installed_hub_identifiers(profile: Optional[str] = None) -> dict:
    ...
    """
    try:
        from hermes.tools.skills_hub import HubLockFile
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    """
    from hermes.runtime.constants import set_hermes_home_override, reset_hermes_home_override
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from hermes.runtime.constants import set_hermes_home_override, reset_hermes_home_override
    from hermes.channels.cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    """
    from hermes.runtime.constants import set_hermes_home_override, reset_hermes_home_override
    from hermes.channels.cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    from hermes.tools import skills_tool as _skills_tool
    from hermes.tools import skill_manager_tool as _skill_mgr
...
def _clear_skills_prompt_cache() -> None:
    ...
    """
    try:
        from hermes.agent.prompt_builder import clear_skills_system_prompt_cache
...
from hermes.channels.cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from hermes.channels.cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from hermes.channels.cli.tools_config import (
...
def _probe_modal_backend() -> tuple:
    try:
        from hermes.tools.tool_backend_helpers import has_direct_modal_credentials
...
if sys.platform.startswith("win"):
    try:
        from hermes.channels.cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from hermes.channels.cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from hermes.channels.cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_client_reason(ws: "WebSocket") -> Optional[str]:
    ...
    so a "WS keeps closing" report can be diagnosed from hermes.agent.log without a
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from hermes.channels.cli.dashboard_auth.audit import AuditEvent, audit_log
        from hermes.channels.cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    """
    from hermes.channels.cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    try:
        from hermes.channels.cli.config import apply_terminal_config_to_env
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from hermes.channels.cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from hermes.channels.cli.console_engine import HermesConsoleEngine
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    from hermes.channels.tui.ws import handle_ws
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    """
    from hermes.channels.cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_dashboard_plugins() -> list:
    ...
    from hermes.channels.cli.plugins import get_bundled_plugins_dir
...
async def get_dashboard_plugins():
    ...
    # from hermes.integrations.plugins the user has not explicitly activated.  (#46435)
    try:
        from hermes.channels.cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _schedule_check_fn_probe(fn) -> Optional[threading.Thread]:
    ...
    def _probe():
        try:
            from hermes.tools.registry import _check_fn_cached
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    for name, version, description, source, dir_str, key in _discover_all_plugins():
        ...
        if provides_tools:
            try:
                from hermes.tools.registry import get_cached_check_fn_result, registry
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from hermes.channels.cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from hermes.channels.cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from hermes.channels.cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from hermes.channels.cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    """Import and mount backend API routes from hermes.integrations.plugins that declare them.
...
from hermes.channels.cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    try:
        from hermes.channels.cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    if app.state.auth_required:
        ...
        from hermes.channels.cli.dashboard_auth import list_providers
        if not list_providers():
            ...
            try:
                from hermes.integrations.plugins.dashboard_auth import nous as _nous_plugin
            ...
            # (#54489).
            try:
                from hermes.channels.cli.config import load_config as _load_cfg
                from hermes.channels.cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "configured.\n"
                        "Remove 'basic' from hermes.integrations.plugins.disabled (or run "
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            try:
                from hermes.channels.tui.loop_noise import install_loop_noise_filter
