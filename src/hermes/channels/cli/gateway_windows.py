...
def _current_profile_cli_args() -> list[str]:
    ...
    from hermes.channels.cli.gateway import _profile_arg
...
def get_task_name() -> str:
    ...
    from hermes.channels.cli.gateway import _profile_suffix
...
def windowless_gateway_restart_spec(
    ...
    from hermes.channels.cli.config import get_hermes_home
    from hermes.channels.cli.gateway import PROJECT_ROOT
...
def _prompt_install_choices(
    ...
    from hermes.channels.cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from hermes.channels.cli.gateway import find_gateway_pids, _profile_arg
...
def _print_deep_probes() -> None:
    ...
    if lock_present:
        try:
            from hermes.channels.gateway.status import is_gateway_runtime_lock_active
...
def _drain_gateway_pid(pid: int, drain_timeout: float) -> bool:
    ...
    if pid <= 0:
        return False
    try:
        from hermes.channels.gateway.status import write_planned_stop_marker, _pid_exists
    except ImportError:
        return False
...
def _force_terminate_known_gateway_pids(pids: list[int]) -> int:
    ...
    try:
        from hermes.channels.gateway.status import _pid_exists, terminate_pid
    except ImportError:
        return 0
