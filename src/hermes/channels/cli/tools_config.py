...
from hermes.channels.cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from hermes.channels.cli.nous_account import format_nous_portal_entitlement_message
from hermes.tools.tool_backend_helpers import fal_key_is_configured
from hermes.runtime.utils import base_url_hostname, is_truthy_value
...
from hermes.channels.cli.cli_output import (  # noqa: E402 — late import block
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from hermes.channels.cli.auth import _read_xai_oauth_tokens
        ...
        pass
    try:
        from hermes.tools.xai_http import get_env_value as _xai_get_env_value
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from hermes.channels.cli.plugins import discover_plugins, get_plugin_toolsets
...
from hermes.channels.cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "browser": {
        ...
        # injected at runtime from hermes.integrations.plugins.browser.<vendor>.provider via
...
def _resolved_cua_driver_cmd() -> Optional[str]:
    ...
    from hermes.tools.computer_use.cua_backend import resolve_cua_driver_cmd
...
def _cua_driver_env() -> dict:
    ...
    """
    try:
        from hermes.tools.computer_use.cua_backend import cua_driver_child_env
...
def _pip_install(
    ...
    *,
    ...
    from hermes.channels.cli.managed_uv import ensure_uv
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    if binary:
        _state = None
        try:
            from hermes.tools.computer_use.cua_backend import cua_driver_update_check
...
def _run_post_setup(post_setup_key: str):
    ...
    import shutil
    from hermes.runtime.constants import find_node_executable
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                import hermes.tools.browser_tool as _bt
        ...
        try:
            from hermes.channels.cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from hermes.channels.cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _get_platform_tools(
    ...
    *,
    ...
    from hermes.tools.toolsets import resolve_toolset, TOOLSETS
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key == "vision":
        try:
            from hermes.agent.auxiliary_client import resolve_vision_provider_client
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    try:
        ...
        from hermes.agent import model_tools  # noqa: F401
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from hermes.channels.cli.curses_ui import curses_checklist
    from hermes.tools.toolsets import resolve_toolset
...
def _plugin_image_gen_providers() -> list[dict]:
    ...
    """
    try:
        from hermes.agent.image_gen_registry import list_providers
...
def _plugin_video_gen_providers() -> list[dict]:
    ...
    """
    try:
        from hermes.agent.video_gen_registry import list_providers
        from hermes.channels.cli.plugins import _ensure_plugins_discovered
...
def _plugin_web_search_providers() -> list[dict]:
    ...
    """
    try:
        from hermes.agent.web_search_registry import list_providers as _list_web_providers
        from hermes.channels.cli.plugins import _ensure_plugins_discovered
...
def web_provider_capabilities(backend: str) -> list:
    ...
    """
    try:
        from hermes.agent.web_search_registry import get_provider
...
def _plugin_browser_providers() -> list[dict]:
    ...
    """
    try:
        from hermes.agent.browser_registry import list_providers as _list_browser_providers
        from hermes.channels.cli.plugins import _ensure_plugins_discovered
...
def _plugin_tts_providers() -> list[dict]:
    ...
    """
    try:
        from hermes.agent.tts_registry import _BUILTIN_NAMES, list_providers
        from hermes.channels.cli.plugins import _ensure_plugins_discovered
...
def _agent_browser_installed() -> bool:
    ...
    from hermes.channels.cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from hermes.channels.cli.nous_subscription import _has_agent_browser
...
def _fal_model_catalog():
    ...
    from hermes.tools.image_generation_tool import FAL_MODELS, DEFAULT_MODEL
...
def _plugin_image_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from hermes.agent.image_gen_registry import get_provider
        from hermes.channels.cli.plugins import _ensure_plugins_discovered
...
def _plugin_video_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from hermes.agent.video_gen_registry import get_provider
        from hermes.channels.cli.plugins import _ensure_plugins_discovered
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    """
    try:
        from hermes.channels.cli.inventory import (
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    try:
        from hermes.tools.mcp_tool import probe_mcp_server_tools
