...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from hermes.channels.cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    with _profile_scope(profile):
        ...
        if name == "web":
            ...
            try:
                from hermes.tools.web_tools import _get_extract_backend, _get_search_backend
...
async def get_computer_use_status(profile: Optional[str] = None):
    ...
    ``permissions status``).
    """
    from hermes.tools.computer_use.permissions import computer_use_status
