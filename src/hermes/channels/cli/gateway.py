...
from hermes.channels.gateway.config import coerce_systemd_watchdog_seconds, load_gateway_config
from hermes.channels.gateway.status import terminate_pid
...
from hermes.channels.cli.setup import (
    print_header,
    print_info,
    print_success,
    print_warning,
...
def find_gateway_pids(
    ...
    if not all_profiles:
        try:
            from hermes.channels.gateway.status import get_running_pid
...
def find_profile_gateway_processes(
    ...
    try:
        from hermes.channels.gateway.status import get_running_pid
        from hermes.channels.cli.profiles import list_profiles
    except Exception:
        return processes
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    try:
        from hermes.channels.gateway.status import looks_like_gateway_command_line
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    from hermes.channels.cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from hermes.channels.cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # base interpreter needs to from hermes.channels import cli as hermes_cli.  Empty on POSIX, where
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from hermes.channels.cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from hermes.channels.cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    from hermes.channels.gateway.status import _pid_exists, write_planned_stop_marker
...
def stop_profile_gateway() -> bool:
    ...
    """
    try:
        from hermes.channels.gateway.status import get_running_pid, remove_pid_file
    except ImportError:
        return False
    ...
    try:
        from hermes.channels.gateway.status import write_planned_stop_marker
...
from hermes.runtime.constants import is_container, is_termux, is_wsl
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            from hermes.runtime.constants import venv_python_path
        except ImportError:
            ...
            from hermes.channels.cli.managed_uv import _reload_hermes_constants
...
def _append_node_dir_for_service(
    ...
    *hermes_root* is the Hermes home the unit will run against. System units
    ...
    """
    from hermes.runtime.constants import iter_hermes_node_dirs
...
def systemd_stop(system: bool = False):
    ...
    _sync_hermes_home_from_systemd_unit(system=system)
    try:
        from hermes.channels.gateway.status import get_running_pid, write_planned_stop_marker
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from hermes.runtime.constants import display_hermes_home as _dhh
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        ...
        from hermes.channels.gateway.status import get_running_pid as _default_running_pid  # noqa
    except Exception:
        return

    try:
        import yaml as _yaml
        from hermes.channels.gateway.status import _read_pid_record  # type: ignore
        ...
        if not rec:
            return
        from hermes.channels.gateway.status import _pid_exists, _pid_from_record
        ...
        # to config.yaml.
        from hermes.channels.gateway.config import _env_multiplex_profiles_override
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    try:
        ...
        from hermes.channels.gateway.status import record_start_and_check_storm
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        ...
        from hermes.channels.gateway.run import _exit_after_graceful_shutdown
...
def _platform_status(platform: dict) -> str:
    ...
    if entry is not None:
        ...
        if entry.is_connected is not None:
            try:
                from hermes.channels.gateway.config import PlatformConfig
...
def _runtime_health_lines() -> list[str]:
    ...
    if gateway_state == "startup_failed" and exit_reason:
        ...
        from hermes.channels.gateway.status import parse_active_agents
...
def _setup_standard_platform(platform: dict):
    ...
    from hermes.channels.cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from hermes.channels.cli.telegram_managed_bot import (
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from hermes.channels.cli import gateway_windows
...
def _setup_weixin():
    ...
    try:
        from hermes.channels.gateway.platforms.weixin import check_weixin_requirements, qr_login
    except Exception as exc:
...
def _setup_qqbot():
    ...
    if method_idx == 0:
        ...
        try:
            from hermes.channels.gateway.platforms.qqbot import qr_register
...
def _builtin_setup_fn(key: str):
    ...
    """
    from hermes.channels.cli import setup as _s
