...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from hermes.channels.cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from hermes.channels.cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from hermes.channels.cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from hermes.channels.cli.gateway import _probe_systemd_service_running
...
class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from hermes.channels.cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from hermes.channels.cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from hermes.channels.cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from hermes.channels.cli.gateway import _probe_launchd_service_running
...
class S6ServiceManager:
    ...
    def _render_finish_script() -> str:
        ...
        normally.  See #51228.
        """
        from hermes.channels.gateway.restart import GATEWAY_FATAL_CONFIG_EXIT_CODE
