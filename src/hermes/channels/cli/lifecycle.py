...
def invoke_hook(hook_name: str, **kwargs: Any) -> List[Any]:
    ...
    try:
        from hermes.channels.cli.observability import observe_lifecycle

        observe_lifecycle(hook_name, **kwargs)
    except Exception:
        logger.warning("Built-in observability hook failed", exc_info=True)

    from hermes.integrations import plugins
...
def has_hook(hook_name: str) -> bool:
    ...
    try:
        from hermes.channels.cli.observability import handles_hook
