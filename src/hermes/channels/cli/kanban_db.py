...
from hermes.channels.cli.sqlite_util import add_column_if_missing as _add_column_if_missing
from hermes.tools.toolsets import get_toolset_names
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    """
    try:
        from hermes.agent.delegation_context import is_delegated_child_process_context
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    """
    from hermes.channels.cli.sqlite_safe_read import connect_tracked
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from hermes.channels.cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # rescue.
    from hermes.channels.cli.sqlite_safe_read import has_live_connection
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    with _cross_process_init_lock(path):
        ...
        from hermes.state.service import preflight_db_writability
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from hermes.channels.cli.sqlite_safe_read import file_length_matches_header
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from hermes.channels.cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    if project_id:
        from hermes.channels.cli import projects_db as _pdb
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from hermes.channels.cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from hermes.channels.cli.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from hermes.channels.cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _resolve_hermes_argv() -> list[str]:
    ...
    local (not imported from hermes.channels.gateway) because ``hermes_cli`` sits below
...
def _default_spawn(
    ...
    *,
    ...
    from hermes.channels.gateway.session_context import _VAR_MAP
    for key in _VAR_MAP:
        env.pop(key, None)
    ...
    from hermes.channels.cli.profiles import resolve_profile_env
    try:
