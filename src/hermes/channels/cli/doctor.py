...
from hermes.runtime.constants import agent_browser_runnable
...
from hermes.channels.cli.colors import Colors, color
from hermes.channels.cli.models import _HERMES_USER_AGENT
from hermes.channels.cli.vercel_auth import describe_vercel_auth
from hermes.runtime.constants import OPENROUTER_MODELS_URL
...
from hermes.runtime.constants import is_termux as _is_termux
...
def _honcho_is_configured_for_doctor() -> bool:
    ...
    try:
        from hermes.integrations.plugins.memory.honcho.client import HonchoClientConfig
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from hermes.channels.cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from hermes.channels.cli.auth import get_xai_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """
    try:
        from hermes.channels.cli import __version__ as init_version
    except Exception:
        return
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    """
    try:
        from hermes.channels.cli.service_manager import (
            S6ServiceManager,
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    try:
        ...
        from hermes.agent.errors import SSLConfigurationError
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    """
    try:
        from hermes.channels.cli.gateway import (
            get_systemd_linger_status,
            get_systemd_unit_path,
            is_linux,
        )
        from hermes.channels.cli.service_manager import detect_service_manager
    except Exception as e:
        check_warn("Gateway service linger", f"(could not from hermes.channels import gateway helpers: {e})")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        from hermes.integrations.providers import list_providers
        from hermes.integrations.providers.base import ProviderProfile as _PP
        try:
            from hermes.channels.cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    try:
        import sqlite3
        from hermes.state.service import is_sqlite_wal_reset_vulnerable, sqlite_source_id
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from hermes.channels.cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from hermes.channels.cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from hermes.channels.cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from hermes.channels.cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from hermes.channels.cli.config import check_config_version, migrate_config
        ...
        # bridge normally derives HERMES_MAX_ITERATIONS from hermes.agent.max_turns
        ...
        try:
            from hermes.channels.cli.config import load_env, read_user_config_raw, remove_env_value
        ...
        try:
            from hermes.channels.cli.config import load_env as _load_env_depr
            from hermes.channels.cli.config import read_user_config_raw as _read_raw_depr
        ...
        try:
            from hermes.channels.cli.config import validate_config_structure
    ...
    try:
        from hermes.channels.cli.config import load_config
        from hermes.channels.cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
    ...
    if state_db_path.exists():
        try:
            ...
            from hermes.state.service import _db_opens_cleanly, repair_state_db_schema
            ...
            from hermes.state.service import is_malformed_db_error, repair_state_db_schema
    ...
    try:
        from hermes.runtime.constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        from hermes.tools.terminal_tool import _SUPPORTED_VERCEL_RUNTIMES
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                from hermes.tools.browser_tool import (
                    _chromium_installed,
                    _is_camofox_mode,
                    _get_cloud_provider,
    ...
    def _probe_anthropic() -> _ConnectivityResult:
        from hermes.channels.cli.auth import get_anthropic_key
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            if base and base.rstrip("/").endswith("/anthropic"):
                from hermes.agent.auxiliary_client import _to_openai_base_url
    ...
    def _probe_azure_entra() -> _ConnectivityResult:
        ...
        try:
            from hermes.agent.azure_identity_adapter import (
                EntraIdentityConfig,
    ...
    try:
        ...
        from hermes.agent.model_tools import check_tool_availability, TOOLSET_REQUIREMENTS
    ...
    try:
        from hermes.channels.cli.config import read_user_config_raw as _read_raw_mem
    ...
    if not _active_memory_provider:
        ...
        try:
            from hermes.integrations.plugins.memory.honcho.client import HonchoClientConfig, resolve_config_path
            ...
            if not _honcho_cfg_path.exists():
                ...
                from hermes.integrations.plugins.memory.honcho.client import get_honcho_client, reset_honcho_client
                reset_honcho_client()
                try:
        ...
        try:
            from hermes.integrations.plugins.memory.mem0 import _load_config as _load_mem0_config
    ...
    try:
        from hermes.channels.cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
