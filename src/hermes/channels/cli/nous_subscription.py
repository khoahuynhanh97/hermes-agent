...
from hermes.channels.cli.config import get_env_value, load_config
...
def _has_agent_browser() -> bool:
    ...
    from hermes.runtime.constants import agent_browser_runnable, with_hermes_node_path
...
def _local_browser_runnable() -> bool:
    ...
    if not _has_agent_browser():
        return False
    try:
        from hermes.tools.browser_tool import _chromium_installed, _using_lightpanda_engine
    except Exception:
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("HERMES_LOCAL_STT_COMMAND"):
        return True
    try:
        from hermes.tools.transcription_tools import _HAS_FASTER_WHISPER
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from hermes.channels.cli.setup import prompt_checklist
    except Exception:
        return set()
