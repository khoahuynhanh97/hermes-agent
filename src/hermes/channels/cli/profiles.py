...
def create_profile(
    ...
    if not soul_path.exists():
        try:
            from hermes.channels.cli.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-c",
             "import json; from hermes.tools.skills_sync import sync_skills; "
...
def _stop_profile_backends(canon: str, profile_dir: Path) -> None:
    ...
    try:
        from hermes.channels.gateway.status import _pid_exists, terminate_pid as _terminate_pid
    except Exception:
        return
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from hermes.channels.cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from hermes.channels.cli.gateway import get_service_name, get_launchd_plist_path
