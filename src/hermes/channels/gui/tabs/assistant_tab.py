...
from hermes.channels.gui.components import LabeledTextbox
...
class AssistantTab:
    ...
    def run_assistant_plan(self):
        ...
        if not request:
            ...
            return
        import uuid
        from hermes.security.ingress import principal_scope, build_gui_principal
        session_id = f"gui_turn_{uuid.uuid4().hex[:8]}"
        with principal_scope(build_gui_principal(session_id)):
