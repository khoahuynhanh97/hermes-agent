...
from hermes.tools.environments.base import BaseEnvironment, _popen_bash
from hermes.tools.environments.local import (
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    try:
        ...
        from hermes.agent.proxy_sources import iron_proxy as ip
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        from hermes.tools.environments.base import get_sandbox_dir
        ...
        try:
            from hermes.tools.credential_files import (
        ...
        if egress_env_overrides:
            try:
                from hermes.channels.cli.config import load_config as _load_cfg_for_collision
            ...
            try:
                from hermes.agent.proxy_sources import iron_proxy as _ip_for_mappings
        ...
        try:
            from hermes.channels.cli.config import load_config as _load_cfg_for_precedence
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        try:
            ...
            from hermes.agent.secret_scope import _is_global_env, is_multiplex_active as _is_multiplex_active
