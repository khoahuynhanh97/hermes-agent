...
async def _download_to_bytes(url: str) -> bytes:
    ...
    from hermes.tools.vision_tools import _download_image
...
def _permitted_host_read_target(p: Path, ctx: ResolveContext) -> Optional[Path]:
    ...
    from hermes.tools.credential_files import from_agent_visible_cache_path
...
def _finalize(
    ...
    """
    from hermes.tools.vision_tools import _detect_image_mime_type_from_bytes
...
def _detect_video_mime(data: bytes, src: str) -> Optional[str]:
    ...
    from hermes.tools.vision_tools import _detect_video_mime_type
