...
from hermes.tools.environments.local import _find_shell, _resolve_safe_cwd, _sanitize_subprocess_env
...
class ProcessRegistry:
    ...
    def __init__(self):
        ...
        try:
            from hermes.tools.async_delegation import restore_undelivered_completions
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from hermes.channels.cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_local(
        ...
        from hermes.tools.terminal_tool import _rewrite_compound_background as _rewrite_bg
...
def _redact_process_result(result: dict) -> dict:
    ...
    if not isinstance(result, dict):
        return result
    from hermes.agent.redact import redact_sensitive_text, redact_terminal_output
