...
Usage:
    from hermes.tools.tts_tool import text_to_speech_tool, check_tts_requirements
...
def get_env_value(name, default=None):
    ...
    """
    try:
        from hermes.channels.cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    """
    try:
        from hermes.tools.tool_backend_helpers import resolve_provider_secret
...
from hermes.tools.managed_tool_gateway import resolve_managed_tool_gateway
...
from hermes.tools.xai_http import hermes_xai_user_agent
...
def _import_elevenlabs():
    ...
    """
    try:
        from hermes.tools.lazy_deps import FeatureUnavailable, ensure
...
def _import_mistral_client():
    ...
    """
    try:
        from hermes.tools.lazy_deps import ensure
...
def _run_command_tts(
    ...
    """
    from hermes.agent.delegation_context import delegated_child_subprocess_env
...
def _sniff_audio_container(path: str) -> str:
    ...
    """
    from hermes.tools.audio_container import sniff_container
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    from hermes.channels.cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    from hermes.tools.xai_http import resolve_xai_http_credentials
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import hermes.channels.cli as _hermes_cli
...
def text_to_speech_tool(
    ...
    # Detect platform from hermes.channels.gateway env var to choose the best output format.
    ...
    if output_path:
        ...
        from hermes.agent.file_safety import is_write_denied
...
def stream_tts_to_speaker(
    ...
    try:
        ...
        from hermes.tools.tts_streaming import SentenceChunker, resolve_streaming_provider
        ...
        def _playback_worker() -> None:
            ...
            if output_stream is not None:
                ...
                try:
                    from hermes.tools.voice_mode import mark_audio_output_active
                except Exception:
