...
from typing import Any, Coroutine, Dict, List, Optional, Sequence, Set, Tuple
from urllib.parse import urlparse

from hermes.tools.registry import tool_error
...
class SecretScopeError(ValueError):
    """Raised when an interpolated secret is not allowed by secret_allowlist."""
    pass


def _build_safe_env(user_env: Optional[dict], allowed_secret_names=None) -> dict:
    if allowed_secret_names is not None and not isinstance(allowed_secret_names, set):
        raise TypeError("allowed_secret_names must be a set or None")

    try:
        from hermes.channels.cli.env_loader import get_secret_source
    except Exception:
    ...
    for key, value in os.environ.items():
        if (
            ...
            env[key] = value
        elif get_secret_source is not None and get_secret_source(key):
            if allowed_secret_names is not None and key in allowed_secret_names:
                env[key] = value

    if not user_env:
        return env

    allowed_set = allowed_secret_names if allowed_secret_names is not None else set()

    for k, v in user_env.items():
        if isinstance(v, str):
            matches = _ENV_VAR_PATTERN.findall(v)
            for raw_ref in matches:
                var_name = _env_ref_name(raw_ref)
                if var_name not in allowed_set:
                    raise SecretScopeError(f"Secret '{var_name}' is not in secret_allowlist")

            def _repl(match):
                var_name = _env_ref_name(match.group(1))
                return os.environ.get(var_name, match.group(0))

            env[k] = _ENV_VAR_PATTERN.sub(_repl, v)
        else:
            env[k] = v
...
def _cache_mcp_image_block(block) -> str:
    ...
    try:
        from hermes.channels.gateway.platforms.base import cache_image_from_bytes
...
def _cache_mcp_audio_block(block) -> str:
    ...
    try:
        from hermes.channels.gateway.platforms.base import cache_audio_from_bytes
        import mimetypes
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    try:
        from hermes.channels.gateway.platforms.base import cache_document_from_bytes
...
class SamplingHandler:
    ...
    async def __call__(self, context, params):
        ...
        from hermes.agent.auxiliary_client import call_llm
...
class ElicitationHandler:
    ...
    async def __call__(self, context, params):
        ...
        try:
            from hermes.tools.approval import request_elicitation_consent
...
class MCPServerTask:
    ...
    async def _refresh_tools(self):
        ...
        """
        from hermes.tools.registry import registry, BatchRegistrationCandidate
        ...
        async with self._refresh_lock:
            ...
            candidates = collect_live_candidates(self.name, self, new_mcp_tools, self._config)
            ownership_snapshot = registry.get_tool_to_toolset_map()
            res = validate_registration_candidates(candidates, ownership_snapshot)

            if res.rejected:
                reasons = [r.reason for r in res.rejected]
                logger.error("MCP server '%s': refresh failed due to rejected candidates: %s", self.name, reasons)
                raise ValueError(f"MCP server '{self.name}': refresh rejected candidates: {reasons}")
            ...
            committed_names = registry.commit_toolset_batch(
                toolset=toolset_name,
                candidates=batch_cands,
                replace_names=old_tool_names,
                aliases={self.name: toolset_name} if batch_cands else None,
            )

            committed_set = set(committed_names)
            with _lock:
                for stale_name in old_tool_names - committed_set:
                    _mcp_tool_server_names.pop(stale_name, None)
                for cname in committed_names:
                    _mcp_tool_server_names[cname] = self.name

                self._tools = new_mcp_tools
                self._registered_tool_names = list(committed_names)
            ...
            try:
                from hermes.tools.mcp_schema_cache import config_fingerprint, write_cache_entry
                tools_filter = self._config.get("tools") or {}
                include_set = _normalize_name_filter(tools_filter.get("include"), f"mcp_servers.{self.name}.tools.include")
                exclude_set = _normalize_name_filter(tools_filter.get("exclude"), f"mcp_servers.{self.name}.tools.exclude")
                ...
                tools_payload = [
                    {
                        ...
                        "inputSchema": getattr(t, "inputSchema", {}) if isinstance(getattr(t, "inputSchema", None), dict) else {},
                    }
                    for t in new_mcp_tools
                    if _should_register(t.name)
                ]
                utility_payload = [
                    {"schema": u["schema"], "handler_key": u["handler_key"]}
                    for u in _select_utility_schemas(self.name, self, self._config)
                ]
                write_cache_entry(self.name, config_fingerprint(self._config), tools=tools_payload, utility_tools=utility_payload)
            except Exception as exc:
                logger.debug("MCP schema cache write failed for '%s': %s", self.name, exc)
    ...
    async def _wait_for_lifecycle_event(self) -> str:
        ...
        The cadence is ``keepalive_interval`` from hermes.channels.api config (default
    ...
    async def _run_stdio(self, config: dict):
        ...
        raw_allowlist = config.get("secret_allowlist")
        allowed_secret_names = set()
        if raw_allowlist is not None:
            if not isinstance(raw_allowlist, list):
                raise TypeError(f"MCP server '{self.name}' secret_allowlist must be a list")
            for item in raw_allowlist:
                if not isinstance(item, str):
                    raise ValueError(f"MCP server '{self.name}' secret_allowlist elements must be strings")
            allowed_secret_names = set(raw_allowlist)

        safe_env = _build_safe_env(user_env, allowed_secret_names=allowed_secret_names)
        ...
        from hermes.tools.osv_check import check_package_for_malware
        try:
...
def _get_auth_error_types() -> tuple:
    ...
    try:
        ...
        pass
    try:
        from hermes.tools.mcp_oauth import OAuthNonInteractiveError
...
def _wrap_with_dashboard_oauth_flow(coro):
    ...
    try:
        from hermes.tools.mcp_dashboard_oauth import (
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    """
    from hermes.tools.interrupt import is_interrupted
    from hermes.agent.async_utils import safe_schedule_threadsafe
...
def _interpolate_env_vars(value):
    ...
    """
    from hermes.agent.secret_scope import get_secret as _get_secret
...
def _filter_suspicious_mcp_servers(servers: Dict[str, dict]) -> Dict[str, dict]:
    ...
    try:
        from hermes.channels.cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
    except Exception:
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    try:
        ...
        from hermes.runtime.utils import env_var_enabled as _env_enabled
...
def _normalize_mcp_input_schema(schema: dict | None) -> dict:
    ...
    * MCP/Pydantic optional fields commonly arrive as
    ...
    def _strip_nullable_union(node):
        ...
        """
        from hermes.tools.schema_sanitizer import strip_nullable_unions
...
class RegistrationResult:
    ...
    rejected: Tuple[RegistrationRejected, ...]
...
def collect_live_candidates(
    ...
    tools_filter = config.get("tools") or {}
    include_set = _normalize_name_filter(tools_filter.get("include"), f"mcp_servers.{server_name}.tools.include")
    exclude_set = _normalize_name_filter(tools_filter.get("exclude"), f"mcp_servers.{server_name}.tools.exclude")
    ...
    tool_timeout = getattr(server, "tool_timeout", _DEFAULT_TOOL_TIMEOUT)
    candidates = []

    for mcp_tool in mcp_tools:
        if not _should_register(mcp_tool.name):
            continue
        _scan_mcp_description(server_name, mcp_tool.name, getattr(mcp_tool, "description", "") or "")
        schema = _convert_mcp_schema(server_name, mcp_tool)
        registry_name = schema["name"]
        descriptor = resolver.resolve_mcp(server_name, config, registry_name)
        ...
        candidates.append(RegistrationCandidate(
            ...
            schema=schema,
            handler=_make_tool_handler(server_name, mcp_tool.name, tool_timeout),
            check_fn=check_fn,
    ...
    for entry in _select_utility_schemas(server_name, server, config):
        ...
        candidates.append(RegistrationCandidate(
            ...
            schema=schema,
            handler=handler_factories[handler_key](server_name, tool_timeout),
            check_fn=check_fn,
            descriptor=descriptor,
        ))
...
def validate_registration_candidates(
    candidates: Sequence[RegistrationCandidate],
    ownership_snapshot: Dict[str, str],
) -> RegistrationResult:
    """Pure candidate validator. Does NOT mutate candidates, schemas, or registry state."""
    accepted = []
    rejected = []

    name_map: Dict[str, List[RegistrationCandidate]] = {}
    for cand in candidates:
        norm_name = sanitize_mcp_name_component(cand.registry_name)
        name_map.setdefault(norm_name, []).append(cand)

    unique_candidates: List[RegistrationCandidate] = []
    for norm_name, group in name_map.items():
        if len(group) == 1:
            ...
            is_exact_dup = True
            for c in group[1:]:
                if (c.raw_name != first.raw_name or
                    c.registry_name != first.registry_name or
                    c.origin != first.origin or
                    c.schema != first.schema or
                    c.handler is not first.handler or
                    c.descriptor != first.descriptor):
                    is_exact_dup = False
                    break
            if is_exact_dup:
                unique_candidates.append(first)
            else:
                for c in group:
                    rejected.append(RegistrationRejected(c, f"Ambiguous name collision for '{c.registry_name}'"))

    for cand in unique_candidates:
        if not isinstance(cand.schema, dict):
            rejected.append(RegistrationRejected(cand, "Malformed schema: must be a dict"))
            continue

        if cand.schema.get("name") != cand.registry_name:
            rejected.append(RegistrationRejected(cand, f"Schema name '{cand.schema.get('name')}' mismatch with '{cand.registry_name}'"))
            continue

        if not callable(cand.handler):
            rejected.append(RegistrationRejected(cand, "Missing or non-callable handler"))
            continue

        if "_internal_toolset_name" in cand.schema:
            rejected.append(RegistrationRejected(cand, "Schema contains internal field '_internal_toolset_name'"))
            continue

        if cand.descriptor is not None:
            desc_wire = getattr(cand.descriptor, "wire_name", None)
            if desc_wire != cand.registry_name:
                rejected.append(RegistrationRejected(cand, f"Descriptor wire_name '{desc_wire}' mismatch with '{cand.registry_name}'"))
                continue

        expected_toolset = cand.descriptor.toolset if cand.descriptor else None
        existing_owner = ownership_snapshot.get(cand.registry_name)
        if existing_owner and expected_toolset and existing_owner != expected_toolset:
            rejected.append(RegistrationRejected(cand, f"Collides with existing owner '{existing_owner}'"))
            continue
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    candidates = collect_live_candidates(name, server, server._tools, config)
    ownership_snapshot = registry.get_tool_to_toolset_map()
    res = validate_registration_candidates(candidates, ownership_snapshot)
    ...
    registered_names = list(
        registry.commit_toolset_batch(
            toolset=toolset_name,
            candidates=batch_cands,
            aliases={name: toolset_name} if batch_cands else None,
        )
    )
    ...
    if registered_names:
        with _lock:
            for rname in registered_names:
                _mcp_tool_server_names[rname] = name
        ...
        try:
            ...
            utility_payload = [
                {"schema": u["schema"], "handler_key": u["handler_key"]}
                for u in _select_utility_schemas(name, server, config)
            ]
...
def _register_from_cache_sync(name: str, config: dict, entry: dict) -> List[str]:
    ...
    from hermes.tools.registry import registry, BatchRegistrationCandidate
    from hermes.capabilities.policies import CapabilityPolicyResolver
    from hermes.tools.mcp_schema_cache import (
        config_fingerprint,
    ...
    for raw in tools_from_cache_entry(entry):
        ...
        registry_name = schema["name"]
        descriptor = resolver.resolve_mcp(name, config, registry_name)

        candidates.append(RegistrationCandidate(
            registry_name=registry_name,
            raw_name=raw_name,
            origin=f"tool {raw_name!r}",
            schema=schema,
    ...
    for raw in utility_tools_from_cache_entry(entry):
        ...
        if not util_name:
            continue
        descriptor = resolver.resolve_mcp(name, config, util_name)

        candidates.append(RegistrationCandidate(
            registry_name=util_name,
            raw_name=util_name,
            origin=f"generated utility {handler_key!r}",
            schema=schema,
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    lazy_server_count = 0
    try:
        from hermes.tools.mcp_schema_cache import config_fingerprint, get_cached_entry
    ...
    from hermes.tools.interrupt import is_interrupted as _is_interrupted, set_interrupt as _set_interrupt
...
def refresh_agent_mcp_tools(
    ...
    *after* that snapshot — a slow HTTP/OAuth server that misses the bounded
    ...
    """
    from hermes.agent.model_tools import get_tool_definitions
    from hermes.tools.registry import registry
...
def _reinject_post_build_tools(agent, tools_list: list, name_set: set) -> set:
    ...
    try:
        ...
        if callable(get_mem_schemas):
            ...
            from hermes.agent.memory_manager import memory_provider_tools_enabled
