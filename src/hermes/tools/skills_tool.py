...
Usage:
    from hermes.tools.skills_tool import skills_list, skill_view, check_skills_requirements
...
from hermes.agent.skill_utils import (
...
def _skills_scan_signature(dirs_to_scan, disabled) -> tuple:
    ...
    """
    from hermes.agent import skill_utils as _skill_utils
...
def _skill_lookup_path_error(name: str) -> Optional[str]:
    ...
    """
    from hermes.tools.path_security import has_traversal_component
...
def skill_matches_platform(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from hermes.agent.skill_utils import skill_matches_platform as _impl
...
def skill_matches_environment(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from hermes.agent.skill_utils import skill_matches_environment as _impl
    return _impl(frontmatter)
...
def _gateway_setup_hint() -> str:
    try:
        from hermes.channels.gateway.platforms.base import GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE
...
def _parse_frontmatter(content: str) -> Tuple[Dict[str, Any], str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from hermes.agent.skill_utils import parse_frontmatter
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    """
    from hermes.agent.skill_utils import get_all_skills_dirs
...
def _get_disabled_skill_names() -> Set[str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from hermes.agent.skill_utils import get_disabled_skill_names
    return get_disabled_skill_names()


def _get_session_platform() -> str:
    """Resolve the current platform from hermes.channels.gateway session context.
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    3. ``HERMES_SESSION_PLATFORM`` from hermes.channels.gateway session context
    """
    try:
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in all directories with collision check.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled state.

    Returns:
        List of skill metadata dicts.
    """
    from hermes.agent.skill_utils import get_all_skills_dirs, iter_skill_index_files
    from hermes.runtime.resources import get_skills_dir as get_bundled_skills_dir
    ...
    # Determine which directory is the bundled skills root
    try:
        bundled_root = get_bundled_skills_dir().resolve()
    except Exception:
        bundled_root = None

    # First pass: find bundled skills and record their names
    bundled_names = set()
    for scan_dir in dirs_to_scan:
        try:
            is_bundled = (bundled_root is not None and scan_dir.resolve() == bundled_root)
        except Exception:
            is_bundled = False
        if not is_bundled:
            continue
    ...
    for scan_dir in dirs_to_scan:
        ...
        for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
            ...
            try:
                ...
                if not is_bundled and name in bundled_names:
                    # Collision detected! Fail closed.
                    logger.error(
                        f"Collision detected! Custom skill at {skill_md} "
                        f"attempts to override bundled skill '{name}'. Fail-closed."
                    )
                    continue
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from hermes.channels.cli.plugins import _get_disabled_plugins, get_plugin_manager
    ...
    if preprocess:
        try:
            from hermes.agent.skill_preprocessing import preprocess_skill_content
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            from hermes.agent.skill_utils import is_valid_namespace, parse_qualified_name
            from hermes.channels.cli.plugins import discover_plugins, get_plugin_manager
        ...
        from hermes.agent.skill_utils import get_external_skills_dirs
        ...
        from hermes.agent.skill_utils import iter_skill_index_files
        ...
        if file_path and skill_dir:
            from hermes.tools.path_security import validate_within_dir, has_traversal_component
            ...
            try:
                from hermes.tools.skill_manager_tool import mark_background_review_skill_read
        ...
        if available_env_names:
            try:
                from hermes.tools.env_passthrough import register_env_passthrough
        ...
        if required_cred_files_raw:
            try:
                from hermes.tools.credential_files import register_credential_files
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved:
                from hermes.tools.skill_usage import bump_use, bump_view
