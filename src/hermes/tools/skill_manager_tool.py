...
from hermes.runtime.utils import atomic_write_text, is_truthy_value
...
def mark_background_review_skill_read(path: Path) -> None:
    ...
    """
    try:
        from hermes.tools.skill_provenance import is_background_review
...
try:
    from hermes.tools.skills_guard import scan_skill, should_allow_install, format_scan_report
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    """
    try:
        from hermes.tools import skill_usage
...
def _background_review_write_guard(
    ...
    try:
        from hermes.agent.skill_utils import is_external_skill_path
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    {"path": Path} or None.
    """
    from hermes.agent.skill_utils import get_all_skills_dirs, is_excluded_skill_path
...
def _maybe_auto_propose_org_edit(name: str, skill_path: Path) -> Optional[str]:
    ...
    """
    try:
        from hermes.agent.skill_utils import is_org_mirror_path
        from hermes.tools import skills_sync_client as ssc
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    ...
    try:
        ...
        from hermes.agent.skill_utils import is_excluded_skill_path
    except Exception:
        return matches
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    """
    from hermes.agent.file_safety import _resolve_active_profile_name
...
def _resolve_skill_target(skill_dir: Path, file_path: str) -> Tuple[Optional[Path], Optional[str]]:
    ...
    from hermes.tools.path_security import validate_within_dir
...
def _patch_skill(
    ...
    from hermes.tools.fuzzy_match import fuzzy_find_and_replace
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from hermes.tools.fuzzy_match import format_no_match_hint
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    if curator_pass:
        try:
            from hermes.tools.skill_usage import archive_skill
...
def _maybe_debounced_sync_push(skill_name: str) -> None:
    ...
    try:
        from hermes.tools.skill_usage import is_sync_enabled
    ...
    def _fire():
        try:
            from hermes.tools.skills_sync_client import maybe_push_skills
...
def skill_manage(
    ...
    if result.get("success"):
        ...
        try:
            from hermes.tools.skill_usage import bump_patch, forget, record_created
            from hermes.tools.skill_provenance import is_background_review
            if action == "create":
                record_created(
                    name,
