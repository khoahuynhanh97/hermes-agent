...
Usage:
    from hermes.tools.browser_tool import browser_navigate, browser_snapshot, browser_click
...
from hermes.agent.redact import redact_cdp_url
from hermes.runtime.constants import (
    agent_browser_runnable,
    get_hermes_home,
    get_hermes_home_override,
)
from hermes.runtime.utils import env_int, is_truthy_value
from hermes.channels.cli.config import DEFAULT_CONFIG, cfg_get
...
def __getattr__(name: str):
    ...
    if name == "requests":
        ...
        return _requests
    if name == "call_llm":
        from hermes.agent.auxiliary_client import call_llm as _call_llm
...
try:
    from hermes.tools.website_policy import check_website_access
except Exception:
    check_website_access = lambda url: None  # noqa: E731 — fail-open if policy module unavailable

try:
    from hermes.tools.url_safety import (
...
from hermes.agent.browser_provider import BrowserProvider as CloudBrowserProvider  # noqa: F401  (legacy alias)
from hermes.agent.browser_registry import (  # noqa: F401  (test-patchable surface)
    get_provider as _registry_get_browser_provider,
)
from hermes.integrations.plugins.browser.browserbase.provider import (  # noqa: F401  (legacy import surface)
    BrowserbaseBrowserProvider as BrowserbaseProvider,
)
from hermes.integrations.plugins.browser.browser_use.provider import (  # noqa: F401
    BrowserUseBrowserProvider as BrowserUseProvider,
)
from hermes.integrations.plugins.browser.firecrawl.provider import (  # noqa: F401
    FirecrawlBrowserProvider as FirecrawlProvider,
)
from hermes.tools.tool_backend_helpers import normalize_browser_cloud_provider
# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from hermes.tools.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
...
def _get_dialog_policy_config() -> Tuple[str, float]:
    ...
    from hermes.tools.browser_supervisor import (
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    if not cdp_url:
        return
    try:
        from hermes.tools.browser_supervisor import SUPERVISOR_REGISTRY  # type: ignore[import-not-found]
...
def _ensure_browser_plugins_loaded() -> None:
    ...
    """
    try:
        from hermes.channels.cli.plugins import _ensure_plugins_discovered
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        # to avoid pipe hang from hermes.agent-browser daemon inheriting fds.
...
def _reap_orphaned_browser_sessions():
    ...
    for socket_dir in socket_dirs:
        ...
        try:
            from hermes.tools.process_registry import ProcessRegistry
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    try:
        from hermes.channels.cli.dep_ensure import ensure_dependency
...
def _extract_screenshot_path_from_text(text: str) -> Optional[str]:
    """Extract a screenshot file path from hermes.agent-browser human-readable output."""
    if not text:
        return None
...
def _run_browser_command(
    ...
    Returns:
        Parsed JSON response from hermes.agent-browser
    """
    if timeout is None:
                ...
                try:
                    ...
                    if command == "screenshot":
                        ...
                        if recovered_path and Path(recovered_path).exists():
                            ...
                            result = {
                                "success": False,
                                "error": f"Non-JSON output from hermes.agent-browser for '{command}': {raw}"
                            }
                    else:
                        result = {
                            "success": False,
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    import urllib.parse
    from hermes.agent.redact import _PREFIX_RE
    ...
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_type
    ...
    from hermes.agent.display import (
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_console
...
def _camofox_current_page_private_url(tab_id: str, user_id: str) -> Optional[str]:
    ...
    """
    try:
        from hermes.tools.browser_camofox import _post
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from hermes.tools.browser_camofox import _ensure_tab, _post
    try:
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    """
    if _is_camofox_mode():
        from hermes.tools.browser_camofox import camofox_vision
    ...
    try:
        ...
        from hermes.tools.vision_tools import (
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if _is_camofox_mode():
        try:
            from hermes.tools.browser_camofox import camofox_close, camofox_soft_cleanup
...
def _maybe_autoinstall_chromium() -> bool:
    ...
    from hermes.tools.lazy_deps import _allow_lazy_installs
...
def check_browser_vision_requirements() -> bool:
    ...
    if not check_browser_requirements():
        return False
    try:
        from hermes.tools.vision_tools import check_vision_requirements
    except ImportError:
        return False
...
if __name__ == "__main__":
    ...
    print("\n💡 Usage:")
    print("  from hermes.tools.browser_tool import browser_navigate, browser_snapshot")
