...
def runtime_tool_names(catalog_snapshot=None) -> set:
    """Return the set of tool names in the active capability catalog / registry snapshot."""
    if catalog_snapshot is not None:
        if hasattr(catalog_snapshot, "descriptors"):
            return set(catalog_snapshot.descriptors.keys())
        if isinstance(catalog_snapshot, (set, list, tuple)):
            return set(catalog_snapshot)
        if isinstance(catalog_snapshot, dict):
            return set(catalog_snapshot.keys())
...
def validate_bundled_skill_tools(skills_dir: Path, active_tools=None) -> List[str]:
    """Validate bundled skills against a capability catalog snapshot."""
    if active_tools is None:
        active_tools = runtime_tool_names()
    elif hasattr(active_tools, "descriptors"):
        active_tools = set(active_tools.descriptors.keys())
    elif isinstance(active_tools, (list, tuple)):
        active_tools = set(active_tools)
    elif isinstance(active_tools, dict):
        active_tools = set(active_tools.keys())

    if not active_tools:
        return ["Snapshot unavailable: runtime tool registry snapshot is empty or not initialized"]
    ...
    for skill_path in sorted(skills_dir.glob("*")):
        ...
        if hermes_meta.get("documentation_only"):
            continue

        allowed_tools = fm.get("allowed-tools")
        if allowed_tools is None:
            errors.append(f"Skill '{skill_path.name}': missing allowed-tools in frontmatter")
            continue
        ...
        for tool_ref in allowed_tools:
            tool_str = str(tool_ref).strip()
            if tool_str not in active_tools:
                matches = [t for t in active_tools if t.endswith(f"__{tool_str}") or t.endswith(f"-{tool_str}")]
                if len(matches) > 0 or (not tool_str.startswith("mcp__") and "nonexistent" not in tool_str and "not_in_active" not in tool_str):
                    errors.append(f"Skill '{skill_path.name}': ambiguous tool reference '{tool_str}'")
                else:
                    errors.append(f"Skill '{skill_path.name}': unknown tool '{tool_str}'")

        requires_tools = hermes_meta.get("requires_tools", [])
        if isinstance(requires_tools, list):
            for req in requires_tools:
                req_str = str(req).strip()
                if req_str not in allowed_set:
                    errors.append(f"Skill '{skill_path.name}': requires_tools '{req_str}' is not in allowed-tools")
