...
def mix_bgm_with_ducking(
    ...
    """Mix BGM into video with sidechain compression ducking.

    Voice audio triggers BGM volume reduction via FFmpeg sidechaincompress.
    Falls back to simple amix if sidechain filter unavailable.
    BGM is looped if shorter than video. Fade-in/fade-out applied.
    ...
    # Sidechain compression filter: BGM ducked by voice presence
    sidechain_filter = (
        f"[1:a]aloop=loop=-1:size=2e+09,atrim=duration={duration},"
        f"afade=t=in:st=0:d={fade_in},"
        f"afade=t=out:st={fade_out_start:.2f}:d={fade_out},"
        f"volume={bgm_volume:.2f}[bgm];"
        f"[bgm][0:a]sidechaincompress=threshold={duck_threshold}:ratio={duck_ratio}"
        f":attack=5:release=50[ducked];"
    ...
    log("[BGM] Mixing BGM with sidechain compression ducking...")
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
        if result.returncode == 0 and Path(output_path).is_file():
            log(f"[BGM] Sidechain ducking mixed -> {output_path}")
            return output_path
        log(f"[BGM] Sidechain filter failed, falling back to amix: {result.stderr[-200:]}")
    except Exception as e:
        log(f"[BGM] Sidechain filter error: {e}, falling back to amix")
    ...
    try:
        ...
        if result.returncode == 0 and Path(output_path).is_file():
            log(f"[BGM] Fallback amix mixed -> {output_path}")
            return output_path
        log(f"[BGM] Fallback also failed: {result.stderr[-200:]}")
    except Exception as e:
        log(f"[BGM] Fallback exception: {e}")
