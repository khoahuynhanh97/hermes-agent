...
Import chain (circular-import safe):
    tools/registry.py  (no imports from model_tools or tool files)
           ^
    tools/*.py  (import from hermes.tools.registry at module level)
           ^
...
from typing import Callable, Dict, List, Optional, Set, Sequence, Tuple, Mapping, Any
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    for path in sorted(tools_path.glob("*.py")):
        ...
        if registers:
            module_names.append(f"hermes.tools.{path.stem}")
...
def _save_discovery_cache(cache: Dict[str, list]) -> None:
    ...
    if path is None:
        return
    try:
        from hermes.runtime.utils import atomic_json_write  # stdlib+yaml only; no cycle
...
class ToolEntry:
    ...
    __slots__ = (
        ...
        "max_result_size_chars", "dynamic_schema_overrides", "descriptor",
    )

    def __init__(self, name, toolset, schema, handler, check_fn,
                 requires_env, is_async, description, emoji,
                 max_result_size_chars=None, dynamic_schema_overrides=None, descriptor=None):
        self.name = name
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    try:
        ...
        if not is_multiplex_active():
            return None
        from hermes.runtime.constants import get_hermes_home_override
...
class BatchRegistrationCandidate:
    name: str
    toolset: str
    schema: dict
    handler: Callable
...
class BatchCommitError(Exception):
    """Base error for batch commit failures."""


class RegistrationConflictError(BatchCommitError):
    """Raised when a candidate collides with an existing toolset or ownership mismatch."""
...
class ToolRegistry:
        ...
        with self._lock:
            ...
            if descriptor is None:
                try:
                    ...
                    descriptor = resolver.resolve_native(name, toolset)
                except Exception:
                    descriptor = None
    ...
    def commit_toolset_batch(
        ...
        *,
        toolset: str,
        candidates: Sequence[BatchRegistrationCandidate],
        replace_names: Optional[Set[str]] = None,
        aliases: Optional[Mapping[str, str]] = None,
    ) -> Tuple[str, ...]:
        """Validate candidates, replace_names, and aliases under registry lock, committing all-or-none."""
        if not candidates and not replace_names and not aliases:
            return ()

        with self._lock:
            snapshot_tools = self._tools.copy()
            snapshot_checks = self._toolset_checks.copy()
            snapshot_aliases = self._toolset_aliases.copy()
            snapshot_gen = self._generation

            try:
                # 1. Verify candidate toolsets match expected toolset & descriptor integrity
                for cand in candidates:
                    if cand.toolset != toolset:
                        raise RegistrationConflictError(
                            f"Candidate '{cand.name}' toolset '{cand.toolset}' does not match batch target '{toolset}'"
                        )
                    if cand.descriptor is not None:
                        wire_name = getattr(cand.descriptor, "wire_name", None)
                        desc_ts = getattr(cand.descriptor, "toolset", None)
                        if wire_name != cand.name:
                            raise RegistrationConflictError(
                                f"Candidate '{cand.name}' descriptor wire_name '{wire_name}' mismatch"
                            )
                        if desc_ts != toolset:
                            raise RegistrationConflictError(
                                f"Candidate '{cand.name}' descriptor toolset '{desc_ts}' mismatch"
                            )

                # 2. Remove stale entries for refresh
                if replace_names:
                    for name in replace_names:
                        existing = self._tools.get(name)
                        if existing and existing.toolset != toolset:
                            raise RegistrationConflictError(
                                f"Cannot remove stale entry '{name}' owned by toolset '{existing.toolset}' (expected '{toolset}')"
                            )
                        self._tools.pop(name, None)

                # 3. Register candidates & validate no cross-toolset collision
                for cand in candidates:
                    existing = self._tools.get(cand.name)
                    if existing and existing.toolset != toolset:
                        raise RegistrationConflictError(
                            f"Candidate tool '{cand.name}' owned by toolset '{toolset}' collides with existing toolset '{existing.toolset}'"
                        )

                    descriptor = cand.descriptor
                    if descriptor is None:
                        raise RegistrationConflictError(
                            f"Candidate tool '{cand.name}' (toolset '{toolset}') missing required capability descriptor"
                        )

                    entry = ToolEntry(
                        name=cand.name,
                        toolset=cand.toolset,
                        schema=cand.schema,
                        handler=cand.handler,
                        check_fn=cand.check_fn,
                        requires_env=[],
                        is_async=cand.is_async,
                        description=cand.description or cand.schema.get("description", ""),
                        emoji="",
                        descriptor=descriptor,
                    )
                    self._tools[cand.name] = entry
                    if cand.check_fn and toolset not in self._toolset_checks:
                        self._toolset_checks[toolset] = cand.check_fn

                # 4. Update aliases
                if aliases:
                    for alias, target_toolset in aliases.items():
                        if target_toolset != toolset:
                            raise RegistrationConflictError(
                                f"Alias '{alias}' target toolset '{target_toolset}' does not match batch toolset '{toolset}'"
                            )
                        existing_owner = self._toolset_aliases.get(alias)
                        if existing_owner and existing_owner != toolset:
                            raise RegistrationConflictError(
                                f"Alias '{alias}' is already owned by toolset '{existing_owner}'"
                            )
                        self._toolset_aliases[alias] = target_toolset

                # 5. Perform orphan cleanup if toolset has 0 tools remaining
                tools_in_set = [t for t in self._tools.values() if t.toolset == toolset]
                if not tools_in_set:
                    self._toolset_checks.pop(toolset, None)
                    stale_aliases = [k for k, v in self._toolset_aliases.items() if v == toolset]
                    for k in stale_aliases:
                        self._toolset_aliases.pop(k, None)

                # 6. Single generation increment
                self._generation += 1
                return tuple(cand.name for cand in candidates)

            except Exception as exc:
                self._tools = snapshot_tools
                self._toolset_checks = snapshot_checks
                self._toolset_aliases = snapshot_aliases
                self._generation = snapshot_gen
                if isinstance(exc, BatchCommitError):
                    raise
                raise BatchCommitError(f"Batch commit failed: {exc}") from exc
    ...
    def _normalize_handler_result(name: str, result):
        ...
        if isinstance(result, dict):
            if result.get("_multimodal") is True and isinstance(result.get("content"), list):
                return result
            try:
                import json
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str | dict:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        if entry.descriptor is None:
            return tool_error(f"Tool {name} lacks a capability descriptor and cannot be dispatched")

        if getattr(entry.descriptor, "principal_mode", None) == "session":
            from hermes.security import current_principal
            principal = current_principal.get()
            if not principal:
                return tool_error(f"Principal context is required for session-scoped tool {name}")
            args = dict(args)
            args["owner_user_id"] = principal.owner_user_id

        try:
            if entry.is_async:
                from hermes.agent.model_tools import _run_async
            ...
            try:
                from hermes.agent.model_tools import _sanitize_tool_error
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from hermes.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
