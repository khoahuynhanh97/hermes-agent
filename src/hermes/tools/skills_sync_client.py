...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    """
    try:
        from hermes.tools.skill_usage import is_bundled, is_hub_installed, _find_skill_dir
...
def list_synced_skill_names() -> List[str]:
    ...
    """
    try:
        from hermes.tools.skill_usage import load_usage
    except Exception:
        return []
    usage = load_usage() or {}
...
def _all_local_skill_names() -> List[str]:
    ...
    try:
        ...
        for skill_md in root.rglob("SKILL.md"):
            ...
            try:
                from hermes.tools.skill_usage import _read_skill_name
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    ...
    try:
        from hermes.tools.skill_usage import _find_skill_dir
    except Exception:
        return None
    skill_dir = _find_skill_dir(skill_name)
    if skill_dir is None:
...
def pull_skills(
    ...
    *,
    ...
    if remote_manifest:
        try:
            from hermes.tools.skill_usage import set_sync, is_curation_eligible, is_sync_enabled
...
def list_org_skill_names() -> List[str]:
    ...
    names: List[str] = []
    try:
        from hermes.agent.skill_utils import read_active_org_id
...
def _org_baseline_path(org_id: str) -> Path:
    ...
    from hermes.agent.skill_utils import ORG_BASELINE_FILE
...
def _write_active_org_marker(org_id: str) -> None:
    ...
    try:
        from hermes.agent.skill_utils import ORG_ACTIVE_MARKER
...
def _write_org_provenance(org_id: str, data: Dict[str, Any]) -> None:
    ...
    try:
        from hermes.agent.skill_utils import ORG_PROVENANCE_FILE
