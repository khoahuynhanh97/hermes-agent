...
# Import from hermes.scheduling module (will be available when properly installed)
...
from hermes.tools.threat_patterns import INVISIBLE_CHARS as _CRON_INVISIBLE_CHARS
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    if (user_deliver or "").strip().lower() == "local":
        return None
    try:
        from hermes.scheduling.scheduler import _resolve_delivery_targets
...
def _validate_cron_base_url(
    ...
    try:
        ...
        from hermes.runtime.utils import base_url_host_matches, base_url_hostname
    except Exception:
...
def _execute_job_now(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    job_id = job["id"]
    try:
        from hermes.scheduling.scheduler import run_one_job
        ...
        try:
            from hermes.tools.environments.base import get_activity_callback
...
def cronjob(
    ...
    try:
        ...
        if normalized == "create":
            ...
            if context_from:
                from hermes.scheduling.jobs import get_job as _get_job
