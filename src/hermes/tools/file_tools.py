...
from hermes.agent.file_safety import get_read_block_error
from hermes.tools.binary_extensions import has_binary_extension
from hermes.tools.file_operations import (
    ShellFileOperations,
...
def _expand_tilde(path: str) -> str:
    ...
    if not path or "~" not in path:
        return path
    try:
        from hermes.runtime.constants import get_subprocess_home
...
def _uses_container_paths(task_id: str = "default") -> bool:
    try:
        from hermes.tools.terminal_tool import _CONTAINER_BACKENDS
...
def _registered_task_cwd_override(task_id: str = "default") -> str | None:
    ...
    """
    try:
        from hermes.tools.terminal_tool import resolve_task_overrides
...
def _resolve_base_dir(
    ...
    *,
    ...
    from hermes.tools.environments.local import _msys_to_windows_path
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    ...
    if env is None:
        return True
    try:
        from hermes.tools.environments.local import LocalEnvironment
    except ImportError:
        return True
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
    ...
    """
    try:
        from hermes.agent.file_safety import (
    ...
    if cached is not None:
        with _env_lock:
            if task_id in _active_environments:
                ...
                if old_cwd:
                    try:
                        from hermes.tools.terminal_tool import record_session_cwd
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        from hermes.tools.read_extract import ExtractionError, extract_document_text, is_extractable_document
...
def _mark_verification_stale(
    ...
    try:
        ...
        from hermes.agent.verification_evidence import mark_workspace_edited
...
def _check_file_reqs():
    """File tools are always available built-in."""
    return True
