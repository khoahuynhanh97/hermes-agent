...
Usage:
    from hermes.tools.file_operations import ShellFileOperations
    from hermes.tools.terminal_tool import _active_environments
...
from hermes.tools.binary_extensions import BINARY_EXTENSIONS
...
def normalize_read_pagination(offset: Any = DEFAULT_READ_OFFSET,
    ...
    """
    from hermes.tools.tool_output_limits import get_max_lines
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        """
        from hermes.tools.tool_output_limits import get_max_line_length
    ...
    def _escape_shell_arg(self, arg: str) -> str:
        ...
        """
        from hermes.tools.environments.local import _bash_safe_path
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        if error or match_count == 0:
            ...
            from hermes.tools.fuzzy_match import is_already_applied
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        """
        # Import patch parser
        from hermes.tools.patch_parser import parse_v4a_patch, apply_v4a_operations
    ...
    def _lsp_handles_extension(self, ext: str) -> bool:
        ...
        if not ext:
            return False
        try:
            from hermes.agent.lsp.servers import SERVERS
        except Exception:  # noqa: BLE001
            return False
        ext_lower = ext.lower()
        for srv in SERVERS:
    ...
    def _lsp_will_handle(self, path: str) -> bool:
        ...
        if not self._lsp_local_only():
            return False
        try:
            from hermes.agent.lsp import get_service
        except Exception:  # noqa: BLE001
            return False
        try:
            svc = get_service()
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        if pre_content is not None and post_content is not None and pre_content != post_content:
            try:
                from hermes.agent.lsp.range_shift import build_line_shift
        ...
        if not diagnostics:
            return ""
        try:
            from hermes.agent.lsp.reporter import report_for_file, truncate
