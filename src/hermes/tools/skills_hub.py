...
from hermes.tools.skills_guard import (
...
# so external `from hermes.tools.skills_hub import SKILLS_DIR` callers still work.
...
def _ssrf_safe_http_get(url: str, *, timeout: int = 20) -> httpx.Response:
    ...
    from hermes.tools.url_safety import create_ssrf_safe_client
...
def _guarded_http_get(url: str, *, timeout: int = 20) -> Optional[httpx.Response]:
    ...
    from hermes.tools.url_safety import SSRFConnectionBlocked
...
def install_from_quarantine(
    ...
    try:
        from hermes.tools.skill_usage import record_installed
