...
async def _validate_image_url_async(url: str) -> bool:
    ...
    if not _image_url_shape_ok(url):
        return False
    from hermes.tools.url_safety import async_is_safe_url
...
async def _download_image(image_url: str, destination: Path, max_retries: int = 3) -> Path:
    ...
    async def _ssrf_redirect_guard(response):
        ...
        """
        from hermes.tools.url_safety import async_is_safe_url, redirect_target_from_response
...
def _resize_image_for_vision(image_path: Path, mime_type: Optional[str] = None,
    ...
    try:
        ...
        try:
            from hermes.tools.lazy_deps import ensure as _ensure_dep
...
def _should_use_native_vision_fast_path() -> bool:
    ...
    """
    try:
        from hermes.agent.auxiliary_client import _read_main_provider, _read_main_model
        from hermes.agent.image_routing import decide_image_input_mode, _lookup_supports_vision
...
async def _vision_analyze_native(
    ...
    try:
        ...
        from hermes.tools.image_source import (
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            from hermes.agent.file_safety import raise_if_read_blocked
