...
from hermes.runtime.utils import atomic_json_write, base_url_host_matches, base_url_hostname

from hermes.runtime.constants import OPENROUTER_MODELS_URL

logger = logging.getLogger(__name__)

# ``requests`` (with urllib3) costs ~27 ms of the `from hermes.channels.cli import classic_runtime as cli` waterfall and
...
try:
    from hermes.integrations.providers import list_providers as _list_providers
...
def _model_id_matches(candidate_id: str, lookup_model: str) -> bool:
    """Return True if *candidate_id* (from hermes.channels.api) matches *lookup_model* (configured).
...
def get_model_context_length(
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # entries are dropped — never a cached value above the table.
                ...
                try:
                    from hermes.agent.bedrock_adapter import get_bedrock_context_length
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from hermes.channels.cli.models import get_copilot_model_context
    ...
    if effective_provider:
        from hermes.agent.models_dev import lookup_models_dev_context
