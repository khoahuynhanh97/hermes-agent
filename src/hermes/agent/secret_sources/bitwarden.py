...
from hermes.agent.secret_sources._cache import (
    CachedFetch as _CachedFetch,
    DiskCache,
    FetchResult,
    is_valid_env_name as _is_valid_env_name,
)
from hermes.agent.secret_sources.base import ErrorKind, SecretSource
from hermes.agent.secret_sources.base import get_source_environment
...
def _encrypted_disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    from hermes.agent.secret_sources._cache import resolve_cache_home
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    bws failures look like::
        ...
        Error:
           0: Received error message from hermes.channels.api: [400 Bad Request] {"error":"invalid_client"}
...
# ---------------------------------------------------------------------------
# Public entry point — called from hermes.channels.cli.env_loader
# ---------------------------------------------------------------------------
