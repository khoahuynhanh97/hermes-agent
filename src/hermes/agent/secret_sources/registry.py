...
from hermes.agent.secret_sources.base import (
...
def _ensure_builtin_sources() -> None:
    ...
    try:
        from hermes.agent.secret_sources.bitwarden import BitwardenSource
        ...
        logger.warning("Failed to register bundled Bitwarden secret source",
                       exc_info=True)
    try:
        from hermes.agent.secret_sources.onepassword import OnePasswordSource
        ...
        logger.warning("Failed to register bundled 1Password secret source",
                       exc_info=True)
    try:
        from hermes.agent.secret_sources.command import CommandSource
