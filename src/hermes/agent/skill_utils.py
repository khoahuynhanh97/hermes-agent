...
from hermes.runtime.constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        if os.getenv("HERMES_KANBAN_TASK") or os.getenv("HERMES_KANBAN_BOARD"):
            ...
            try:
                from hermes.tools.kanban_tools import _profile_has_kanban_toolset
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: user-owned HERMES_HOME/skills first, then bundled, then external.

    The local ``HERMES_HOME/skills`` dir is always first so user customizations
    win over bundled defaults. Bundled resources come next; external
    ``skills.external_dirs`` last.  When the user dir and a bundled source
    resolve to the same filesystem path the duplicate is dropped (this can
    happen when an operator sets ``HERMES_SKILLS_DIR`` to the same value as
    the bundled cache).
    """
    ...
    try:
        ...
        if bundled.exists():
            ...
            if resolved in seen:
                logger.debug(
                    "Bundled skills dir %s collides with user skills root; "
                    "ignoring bundled copy to avoid double-loading",
                    resolved,
                )
            else:
                seen.add(resolved)
        ...
        # Bundled package data not available; that's fine — user skills and
        # external dirs are still served.
        logger.debug("No bundled skills available; user + external only")
    except Exception as exc:  # pragma: no cover — defensive
        logger.debug("Bundled skills lookup failed: %s", exc)
    for external in get_external_skills_dirs():
        if external.resolve() in seen:
            continue
        seen.add(external.resolve())
