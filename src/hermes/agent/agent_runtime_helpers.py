...
from hermes.agent.prompt_builder import format_steer_marker
from hermes.agent.tool_dispatch_helpers import _trajectory_normalize_msg, make_tool_result_message
from hermes.agent.trajectory import convert_scratchpad_to_think
from hermes.agent.credential_pool import STATUS_EXHAUSTED, credential_pool_matches_provider
from hermes.agent.error_classifier import FailoverReason
from hermes.agent.turn_context import drop_stale_api_content
from hermes.runtime.utils import base_url_host_matches, base_url_hostname, env_var_enabled, atomic_json_write
    ...
    *,
    ...
    if pool_provider and current_provider != pool_provider:
        ...
        if current_provider == "custom" and pool_provider.startswith("custom:"):
            try:
                from hermes.agent.credential_pool import get_custom_provider_pool_key
    ...
    try:
        ...
        if pool is not None and pool.has_available():
            ...
            if entry is not None:
                ...
                from hermes.agent.credential_pool import CUSTOM_POOL_PREFIX
                if (
                    primary_provider == "custom"
        ...
        from hermes.agent.chat_completion_helpers import _reset_stale_streak
        _reset_stale_streak(agent)

        # Undo the fallback's identity rewrite so the prompt is
        # byte-identical to the stored copy again (prefix cache match).
        from hermes.agent.chat_completion_helpers import rewrite_prompt_model_identity
...
def plan_cache_sections_for_destination(
    ...
    *,
    ...
    """
    from hermes.agent.prompt_caching import (
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from hermes.channels.cli.config import load_config as _load_moa_cfg
    ...
    from hermes.agent.anthropic_adapter import _model_name_is_kimi_family
    is_kimi = (
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    from hermes.agent.auxiliary_client import _validate_base_url, _validate_proxy_env_urls
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from hermes.channels.cli.providers import determine_api_mode
    ...
    try:
        ...
        from hermes.channels.cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from hermes.channels.cli.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from hermes.channels.cli.plugins import resolve_pre_tool_block
    ...
    if block_message is not None:
        ...
        try:
            from hermes.agent.model_tools import _emit_post_tool_call_hook
    ...
    if function_name == "todo":
        def _execute(next_args: dict) -> Any:
            from hermes.tools.todo_tool import todo_tool as _todo_tool
        ...
        def _execute(next_args: dict) -> Any:
            ...
            from hermes.tools.session_search_tool import session_search as _session_search
            return _finish_agent_tool(
                _session_search(
        ...
        def _execute(next_args: dict) -> Any:
            ...
            from hermes.tools.memory_tool import memory_tool as _memory_tool
            result = _memory_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from hermes.tools.clarify_tool import clarify_tool as _clarify_tool
            return _finish_agent_tool(
                _clarify_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from hermes.tools.read_terminal_tool import read_terminal_tool as _read_terminal_tool
            return _finish_agent_tool(
                _read_terminal_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from hermes.tools.read_preview_tool import read_preview_tool as _read_preview_tool
            return _finish_agent_tool(
                _read_preview_tool(
    ...
    from hermes.channels.cli.middleware import run_tool_execution_middleware
...
def looks_like_codex_intermediate_ack(
    ...
    from hermes.agent.codex_responses_adapter import _summarize_user_message_for_log
...
def copy_reasoning_content_for_api(agent, source_msg: dict, api_msg: dict) -> None:
    ...
    """
    from hermes.agent.message_sanitization import apply_reasoning_content_policy
...
def reapply_reasoning_echo_for_provider(agent, api_messages: list) -> int:
    ...
    * Switching TO a strict provider that rejects the field (Mistral,
    ...
    removed.
    """
    from hermes.agent.message_sanitization import reapply_reasoning_echo
