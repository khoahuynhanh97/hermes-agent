...
from hermes.agent.model_metadata import MINIMUM_CONTEXT_LENGTH, get_model_context_length
from hermes.channels.cli.config import get_hermes_home
from hermes.runtime.constants import OPENROUTER_BASE_URL
from hermes.runtime.utils import base_url_host_matches, base_url_hostname, env_float, model_forces_max_completion_tokens, normalize_proxy_env_vars
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    """
    try:
        from hermes.agent.ssl_verify import resolve_httpx_verify
...
def _openai_http_client_kwargs(
    ...
    *,
    ...
    try:
        from hermes.agent.process_bootstrap import build_keepalive_http_client
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from hermes.channels.cli.config import cfg_get, load_config
        _cfg = load_config()
...
#
# The tags are computed from hermes.agent.portal_tags so the client= marker stays
...
from hermes.agent.portal_tags import nous_portal_tags as _nous_portal_tags
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from hermes.channels.cli.auth import _nous_inference_env_override
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        from hermes.agent.codex_responses_adapter import _chat_messages_to_responses_input
        from hermes.runtime.utils import base_url_host_matches
        ...
        if tools:
            ...
            try:
                ...
                from hermes.tools.schema_sanitizer import (
        ...
        try:
            from hermes.agent.transports.codex import (
        ...
        try:
            ...
            from hermes.agent.codex_runtime import _consume_codex_event_stream
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        if not self._base_url:
            ...
            if candidate:
                try:
                    from hermes.agent.anthropic_adapter import _is_nous_portal_endpoint
    ...
    def create(self, **kwargs) -> Any:
        from hermes.agent.anthropic_adapter import build_anthropic_kwargs, create_anthropic_message
        ...
        if temperature is not None:
            from hermes.agent.anthropic_adapter import _forbids_sampling_params
...
class _BedrockCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from hermes.agent.bedrock_adapter import call_converse
...
def _maybe_wrap_anthropic(
    ...
    if _safe_isinstance(client_obj, CodexAuxiliaryClient):
        return client_obj
    try:
        from hermes.agent.gemini_native_adapter import GeminiNativeClient
        if _safe_isinstance(client_obj, GeminiNativeClient):
            return client_obj
    except ImportError:
        pass
    try:
        from hermes.agent.copilot_acp_client import CopilotACPClient
...
def _nous_api_key(provider: dict) -> str:
    ...
    from hermes.channels.cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from hermes.channels.cli.auth import _agent_key_is_usable
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from hermes.channels.cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from hermes.channels.cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from hermes.channels.cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if provider_id == "gemini":
                from hermes.agent.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                try:
                    from hermes.integrations.providers import get_provider_profile as _gpf_aux
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            try:
                from hermes.integrations.providers import get_provider_profile as _gpf_aux2
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from hermes.channels.cli.config import cfg_get, load_config_readonly
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from hermes.agent.nous_rate_guard import nous_rate_limit_remaining
    ...
    model = _NOUS_MODEL
    try:
        from hermes.channels.cli.models import get_nous_recommended_aux_model
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from hermes.channels.cli.config import load_config
        from hermes.channels.cli.moa_config import resolve_moa_preset
...
def _relay_sync_completion(
    ...
    *,
    ...
    from hermes.agent import relay_llm
...
def _try_azure_foundry(
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from hermes.channels.cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _try_anthropic(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    try:
        from hermes.agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
    except ImportError:
        return None, None
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from hermes.channels.cli.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "anthropic":
            from hermes.agent.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _replan_synchronous_cache_sections(
    ...
    *,
    ...
    from hermes.agent.agent_runtime_helpers import plan_cache_sections_for_destination
...
def _try_main_agent_model_fallback(
    ...
    from hermes.agent.backend_identity import (
        BackendIdentity,
        FailureScope,
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from hermes.agent.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        try:
            from hermes.agent.model_metadata import _infer_provider_from_url
            from hermes.integrations.providers import get_provider_profile as _gpf_async
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        from hermes.channels.cli.providers import nous_api_mode
    ...
    if provider == "custom":
        ...
        if custom_base and custom_key:
            ...
            if base_url_host_matches(custom_base, "api.kimi.com"):
                ...
                try:
                    from hermes.integrations.providers import get_provider_profile as _gpf_custom
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            try:
                from hermes.integrations.providers import get_provider_profile as _gpf_main
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from hermes.agent.vertex_adapter import get_vertex_config, has_vertex_credentials
        except ImportError:
        ...
        try:
            from hermes.agent.bedrock_adapter import (
                has_aws_credentials,
...
def _resolve_task_provider_model(
    ...
    def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
        ...
        if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
            return False
        try:
            from hermes.channels.cli.providers import get_provider
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    try:
        from hermes.channels.cli.plugins import get_plugin_auxiliary_tasks
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if not _is_gemini_native and _effective_base:
            try:
                from hermes.agent.gemini_native_adapter import is_native_gemini_base_url
    ...
    try:
        from hermes.integrations.providers import get_provider_profile
        from hermes.integrations.providers.base import ProviderProfile
    ...
    if _provider_for_portal in {"nous", "nous-portal", "nousresearch"}:
        ...
        if "session_id" not in merged_extra:
            try:
                from hermes.agent.portal_tags import get_conversation_context
...
def _validate_llm_response(
    ...
    from hermes.agent.aux_accounting import record_aux_usage
