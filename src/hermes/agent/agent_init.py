...
from hermes.agent.memory_manager import StreamingContextScrubber
...
from hermes.agent.process_bootstrap import _install_safe_stdio
from hermes.agent.subdirectory_hints import SubdirectoryHintTracker
from hermes.agent.think_scrubber import StreamingThinkScrubber
from hermes.agent.tool_guardrails import (
    ToolCallGuardrailConfig,
    ToolCallGuardrailController,
    ToolGuardrailDecision,
)
from hermes.channels.cli.config import cfg_get
from hermes.channels.cli.route_identity import normalize_route_base_url
from hermes.channels.cli.timeouts import get_provider_request_timeout
from hermes.runtime.constants import get_hermes_home
from hermes.runtime.utils import base_url_host_matches, is_truthy_value
...
def _ra():
    ...
    """
    from hermes.agent import runtime_agent as run_agent
    return run_agent
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from hermes.channels.cli.providers import HERMES_OVERLAYS, get_provider
    ...
    try:
        ...
        from hermes.channels.cli.models import normalize_provider as normalize_model_provider
        from hermes.channels.cli.providers import normalize_provider as normalize_registry_provider
...
def init_agent(
    ...
    if credential_pool is not None:
        try:
            from hermes.agent.credential_pool import credential_pool_matches_provider
    ...
    agent._cache_ttl = "5m"
    try:
        from hermes.channels.cli.config import load_config_readonly as _load_pc_cfg

        from hermes.agent.agent_runtime_helpers import cache_ttl_means_disabled
    ...
    from hermes.agent.credits_tracker import new_credits_latch
    ...
    from hermes.runtime.logging import setup_logging, setup_verbose_logging
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from hermes.channels.cli.auth import build_minimax_oauth_token_provider
            ...
            from hermes.agent.anthropic_adapter import _is_oauth_token as _is_oat
        ...
        from hermes.agent.moa_loop import build_moa_facade
        agent.api_mode = "chat_completions"
        ...
        try:
            from hermes.channels.cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                from hermes.agent.auxiliary_client import build_or_headers
                client_kwargs["default_headers"] = build_or_headers()
            elif base_url_host_matches(effective_base, "integrate.api.nvidia.com"):
                from hermes.agent.auxiliary_client import build_nvidia_nim_headers
                ...
                try:
                    from hermes.integrations.providers import get_provider_profile as _gpf
            ...
            from hermes.agent.auxiliary_client import resolve_provider_client
        ...
        try:
            from hermes.agent.ssl_guard import verify_ca_bundle_with_fallback
    ...
    from hermes.agent.agent_runtime_helpers import sync_credential_pool_entry_id
    ...
    try:
        from hermes.tools.registry import registry as _snapshot_registry
    ...
    from hermes.agent.prompt_builder import KANBAN_GUIDANCE
    ...
    try:
        ...
        try:
            from hermes.agent.delegation_context import is_delegated_child_context
    ...
    try:
        from hermes.channels.cli.config import load_config_readonly as _load_sess_cfg
    ...
    try:
        from hermes.tools.approval import _YOLO_MODE_FROZEN
    ...
    try:
        from hermes.channels.cli.config import load_config_readonly as _load_agent_config
    ...
    if not skip_memory or _memory_toolset_requested:
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                from hermes.tools.memory_tool import MemoryStore
    ...
    if not skip_memory:
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                from hermes.agent.memory_manager import MemoryManager as _MemoryManager
                from hermes.integrations.plugins.memory import load_memory_provider as _load_mem
    ...
    from hermes.agent.memory_manager import inject_memory_provider_tools as _inject_memory_provider_tools
    ...
    if agent._environment_probe:
        try:
            from hermes.tools.env_probe import warm_environment_probe_async
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from hermes.channels.cli.auth import resolve_provider as resolve_auth_provider
        ...
        if not _configured_base_url and _custom_provider_candidate:
            ...
            if isinstance(_user_providers, dict):
                from hermes.channels.cli.config import is_provider_enabled
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from hermes.channels.cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        # Try loading from hermes.integrations.plugins/context_engine/<name>/
        try:
            from hermes.integrations.plugins.context_engine import load_context_engine
            _selected_engine = load_context_engine(_engine_name)
        except Exception as _ce_load_err:
            _ra().logger.debug("Context engine load from hermes.integrations.plugins/context_engine/: %s", _ce_load_err)

        # Try general plugin system as fallback
        if _selected_engine is None:
            _candidate = None
            try:
                from hermes.channels.cli.plugins import get_plugin_context_engine
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from hermes.channels.cli.model_switch import _check_hermes_model_warning
    ...
    if (
        ...
        from hermes.agent.memory_manager import normalize_tool_schema as _normalize_tool_schema
