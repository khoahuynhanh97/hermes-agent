...
from hermes.tools.toolsets import resolve_toolset, validate_toolset
...
# Import new knowledge tools to register them
try:
    import hermes.knowledge.tools
except ImportError as e:
    logger.warning("Could not import knowledge tools: %s", e)

# Import new product_intelligence tools
try:
    import hermes.product_intelligence.tools
except ImportError as e:
    logger.warning("Could not import product_intelligence tools: %s", e)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from hermes.channels.cli.config import get_config_path
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        from hermes.tools.toolsets import get_all_toolsets
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                from hermes.tools.toolsets import bundle_non_core_tools, get_toolset
    ...
    if "execute_code" in available_tool_names:
        from hermes.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    for discord_tool_name in _discord_schema_fns:
        if discord_tool_name in available_tool_names:
            try:
                from hermes.tools import discord_tool as _dt
    ...
    try:
        from hermes.tools.schema_sanitizer import sanitize_tool_schemas
    ...
    try:
        from hermes.tools.tool_search import assemble_tool_defs, load_config as _load_ts_config
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from hermes.channels.cli.config import load_config as _load
        cfg = _load() or {}
...
def coerce_tool_args(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        from hermes.tools.schema_sanitizer import unrename_tool_args
...
def _tool_result_observer_fields(
    ...
    try:
        ...
        pass
    try:
        from hermes.agent.display import _detect_tool_failure
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from hermes.channels.cli.lifecycle import has_hook, invoke_hook
...
def handle_function_call(
    ...
    _ts_mod = None
    try:
        from hermes.tools import tool_search as _ts_mod  # noqa: F401
    except Exception:
        _ts_mod = None
    ...
    try:
        ...
        try:
            from hermes.channels.acp.edit_approval import maybe_require_edit_approval
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from hermes.tools.file_tools import notify_other_tool_call
