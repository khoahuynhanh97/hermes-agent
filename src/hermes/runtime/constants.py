...
def get_skills_dir() -> Path:
    """Return the path to the skills directory under HERMES_DATA_DIR / skills."""
    data_dir = os.environ.get("HERMES_DATA_DIR", "").strip()
    if data_dir:
        return Path(data_dir).expanduser().resolve() / "skills"
    # Fallback to hermes-agent-data next to repo root
    repo_root = Path(__file__).resolve().parents[3]
    return (repo_root.parent / "hermes-agent-data").resolve() / "skills"
