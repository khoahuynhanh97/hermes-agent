...
def resource_pack_save(owner_user_id: str, project_id: str, resource_pack: dict[str, Any]) -> dict[str, Any]:
...
def creative_brief_save(owner_user_id: str, project_id: str, creative_brief: dict[str, Any]) -> dict[str, Any]:
...
def _service() -> VideoFactoryService:
    ...
    path = Path(configured).expanduser().resolve() if configured else get_data_path("db", "video_factory.sqlite")
    return VideoFactoryService(SQLiteVideoFactoryRepository(Database(path)))


def _resource_pack(value: dict[str, Any], owner: str) -> ResourcePack:
    data = dict(value)
