...
plan. Later jobs can connect these plans to AgentJobManager, Telegram, and a
real coding executor.
"""
...
class HermesAssistantRuntime:
    ...
    def classify(self, message: str) -> str:
        ...
        learning_terms = ["learn", "hoc", "kien thuc", "knowledge"]
        if any(term in text for term in coding_action_terms) and not any(
            term in text for term in learning_terms
        ):
            return "coding_agent"
        scores: dict[str, int] = {}
        for module, keywords in INTENT_RULES:
